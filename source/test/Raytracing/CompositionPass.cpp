...
namespace Moer::Render {
    CompositionPass::CompositionPass(RenderDevice& _device, ShaderManager& _manager, Scene& _scene)
        : device(_device), manager(_manager), scene(_scene), gbuffer_pass_pipeline{manager.Compute<CompositionPassPipeline>("hwrt/CompositionPass.hlsl")} {
        gbuffer_constants = device.CreateBuffer<Moer::byte>(sizeof(CompositingConstants), EBufferUsageFlags::CONSTANT_BUFFER);
        gbuffer_constants->SetName("gbuffer_constants");
    }

    void CompositionPass::Process(
        ...
        _cmd_list.Compute(gbuffer_pass_pipeline,
                          gbuffer_constants,
                          _rt_ctx.frame_rt.resolved_color,
