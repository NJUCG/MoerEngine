...
namespace Moer::Render {
    ...
    */
    ...
    void VulkanDescriptorSetWriter::WriteImage(uint32_t _write_index, VkSampler _sampler, VkImageView _image_view, VkImageLayout _image_layout) {
        auto* image_info = const_cast<VkDescriptorImageInfo*>(m_descriptor_write_head[_write_index].pImageInfo);
        if (_image_layout == VK_IMAGE_LAYOUT_UNDEFINED) {
            _image_layout = DecideImageLayout(m_descriptor_write_head[_write_index].descriptorType);
        }
        image_info->sampler     = _sampler;
        image_info->imageView   = _image_view;
        image_info->imageLayout = _image_layout;
    ...
    void VulkanDescriptorSetWriter::WriteBuffer(uint32_t _write_index, VkBuffer _buffer, VkDeviceSize _offset, VkDeviceSize _range) {
        auto* buffer_info = const_cast<VkDescriptorBufferInfo*>(m_descriptor_write_head[_write_index].pBufferInfo);
    ...
    VulkanDescriptorHeap::VulkanDescriptorHeap(VulkanDevice& _device) : m_device(&_device),
        ...
        VK_CHECK_RESULT(vmaCreateBuffer(_device.GetVmaAllocator(), &buffer_ci, &alloc_ci, &desc_buffer, &desc_buffer_allocation, nullptr));
