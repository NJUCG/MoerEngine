    ...
    public:
        ...
        void EmplaceArg(uint64 _handle, ResourceType _type, const Range& _range, bool _b_write) {
            ...
            if (_b_write) {
                switch (_type) {
                    case ResourceType::Bindless: {
                        m_dispatch_layer = std::max(m_dispatch_layer, GetLastLayerWrite(static_cast<BindlessHandle*>(handle)));
                        break;
                    }
                    case ResourceType::Mesh:
                    case ResourceType::Accel: {
                ...
                switch (_type) {
                    case ResourceType::Bindless: {
                        m_dispatch_layer = std::max(m_dispatch_layer, GetLastLayerRead(static_cast<BindlessHandle*>(handle)));
                        break;
                    }
                    case ResourceType::Mesh:
                    case ResourceType::Accel: {
        ...
        void VisitCmd(const BuildAccelerationStructuresCmd* _cmd) {
            ...
            for (const auto& vtx_buf : _cmd->VtxBuffers()) {
                ...
                layer            = GetLastLayerRead(vtx_handle, Range(0, vtx_buf->GetByteSize()));
            }
            for (const auto& idx_buf : _cmd->IdxBuffers()) {
                ...
                layer            = GetLastLayerRead(idx_handle, Range(0, idx_buf->GetByteSize()));
            }
