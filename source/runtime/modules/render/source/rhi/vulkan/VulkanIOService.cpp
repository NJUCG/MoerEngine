...
namespace Moer::Render {
    ...
    void VulkanCommitSession::Enqueue(FileHandle _handle, size_t _file_offset, void* _ptr, size_t _len) {
        ...
        file_cmds.push_back({FileDesc{_handle, uint(_file_offset), uint(_len)}, RawDataDesc{std::span<ubyte>((ubyte*)_ptr, _len)}, 0});
    }

    void VulkanCommitSession::Enqueue(FileHandle _handle, size_t _file_offset, void* _buffer_ptr, size_t _offset, size_t _len) {
        // Enqueue file to buffer copy
        file_cmds.push_back({FileDesc{_handle, uint(_file_offset), uint(_len)}, BufferViewDesc{uint64(_buffer_ptr), uint(_offset), uint(_len)}, 0});
    }

    void VulkanCommitSession::Enqueue(FileHandle _handle, size_t _file_offset, void* _tex_ptr, EPixelFormat _format, uint3 _offset, uint3 _size, uint _mip_offset) {
        // Enqueue file to texture copy
        file_cmds.push_back({FileDesc{_handle, uint(_file_offset), uint(GetSizeFromImageFormat(_format, _size))}, TextureViewDesc{uint3(_offset), uint3(_size), uint64(_tex_ptr), _format, uint(_mip_offset), 0, 0, 0}, 0});
    }

    void VulkanCommitSession::Enqueue(void const* _mem, size_t _file_offset, void* _buffer_ptr, size_t _offset, size_t _len) {
        // Enqueue memory to buffer copy
        mem_cmds.push_back({RawDataDesc{std::span<ubyte>((ubyte*)_mem, _len)}, BufferViewDesc{uint64(_buffer_ptr), uint(_offset), uint(_len)}, 0});
    }

    void VulkanCommitSession::Enqueue(void const* _mem, size_t _file_offset, void* _tex_ptr, EPixelFormat _format, uint3 _offset, uint3 _size, uint _mip_offset) {
        // Enqueue memory to texture copy
        mem_cmds.push_back({RawDataDesc{std::span<ubyte>((ubyte*)_mem, GetSizeFromImageFormat(_format, _size))}, TextureViewDesc{uint3(_offset), uint3(_size), uint64(_tex_ptr), _format, uint(_mip_offset), 0, 0, 0}, 0});
    }
    ...
    struct VulkanIOTaskThread {
        ...
        VulkanIOTaskThread() {
            thread = std::jthread([this]() { InnerWorkLoop(); });
        }
                    ...
                    for (auto& cmd : task->file_cmds) {
                        ...
                        std::visit(
                            Overload{
                                [&](RawDataDesc& _dst) {
                                    copy_file_to_mem(
                                        std::get<FileDesc>(cmd.src), cmd.file_offset, _dst.data);
                                },
                                [&](BufferViewDesc& _dst) {
                    ...
                    for (auto& cmd : task->file_cmds) {
                        std::visit(
                            Overload{
                                ...
                                [&](BufferViewDesc& _dst) {
                                    copy_file_to_mem(
                                        std::get<FileDesc>(cmd.src), cmd.file_offset, std::span<ubyte>(temp_buffer.data() + temp_size, _dst.size));
                                    VulkanBuffer* buffer = (VulkanBuffer*)_dst.handle;
                                    cmd_list.CopyFrom(std::span<byte>((byte*)temp_buffer.data() + temp_size, _dst.size), buffer->GetView(_dst.offset, _dst.size));
                                    temp_size += cmd.SizeByte();
                                },
                                [&](TextureViewDesc& _dst) {
                                    copy_file_to_mem(
                                        std::get<FileDesc>(cmd.src), cmd.file_offset, std::span<ubyte>(temp_buffer.data() + temp_size, _dst.GetByteSize()));
                                    ...
                                    cmd_list.CopyFrom(std::span<byte>((byte*)temp_buffer.data() + temp_size, _dst.GetByteSize()), view);
                                    temp_size += cmd.SizeByte();
                                }},
                            cmd.dst);
                    }
                        ...
                        std::visit(
                            Overload{
                                ...
                                [&](BufferViewDesc& _dst) {
                                    ...
                                    VulkanBuffer*      buffer = (VulkanBuffer*)_dst.handle;
                                    cmd_list.CopyFrom(std::span<byte>((byte*)src.data.data(), src.data.size_bytes()), buffer->GetView(_dst.offset, _dst.size));
                                },
                                [&](TextureViewDesc& _dst) {
                                    const RawDataDesc& src     = std::get<RawDataDesc>(cmd.src);
                                    VulkanTexture*     texture = (VulkanTexture*)_dst.handle;
                                    ...
                                    view.offset = _dst.offset;
                                    view.extent = _dst.size;
                                    cmd_list.CopyFrom(std::span<byte>((byte*)src.data.data(), src.data.size_bytes()), view);
                                }},
                            cmd.dst);
                    }
    ...
    VulkanIOInterface::VulkanIOInterface(VulkanDevice& _device, VkCopyQueue& _queue) : m_device(_device), m_queue(_queue) {
                                ...
                                for (auto i = 0; i < _dst.size[2]; i++) {
                                    ...
                                    do {
                                        ...
                                        _splited_cmds.emplace_back(
                                            cur_cmd.src,
                                            TextureViewDesc{
                                                uint3(_dst.offset.x, _dst.offset.y + col_offset, _dst.offset.z + i),
                                        ...
                                        if (col_offset < _dst.size.y) {
                                            cur_cmd.file_offset += GetSizeFromImageFormat(_dst.pixel_fmt, uint3(_dst.size.x, dst_col, 1));
                                        } else {
                                            break;
                                        }
                                    } while (true);
                ...
                std::visit(
                    Overload{
                        ...
                        [&](RawDataDesc& _src, TextureViewDesc& _dst) {
                            storage->Enqueue(_src.data.data(), cmd.first.file_offset, (void*)_dst.handle, cmd.first.file_offset, cmd.first.SizeByte());
                        },
                        [&](FileDesc& _src, BufferViewDesc& _dst) {
                            storage->Enqueue(_src.handle, cmd.first.file_offset, (void*)_dst.handle, cmd.first.file_offset, cmd.first.SizeByte());
                        },
                        [&](FileDesc& _src, TextureViewDesc& _dst) {
                            storage->Enqueue(_src.handle, cmd.first.file_offset, (void*)_dst.handle, cmd.first.file_offset, cmd.first.SizeByte());
                        },
                        [&](FileDesc& _src, RawDataDesc& _dst) {
                            storage->Enqueue(_src.handle, cmd.first.file_offset, (void*)_dst.data.data(), cmd.first.file_offset, cmd.first.SizeByte());
                        },
                        [&](auto& _src, auto& _dst) {
    ...
    uint64 VulkanIOInterface::Execute(IOCommandList&& _cmdlist) {
        uint64 timline;
        if (_cmdlist.callbacks.empty() && _cmdlist.files.empty() && _cmdlist.cmds.empty() && _cmdlist.import_cmds.empty() && _cmdlist.export_cmds.empty()) {
    ...
    void
    VulkanStorage::Enqueue(FileHandle _handle, size_t _file_offset, void* _ptr, size_t _len) {
        GetCurrentSession()->Enqueue(_handle, _file_offset, _ptr, _len);
    }
    void VulkanStorage::Enqueue(FileHandle _handle, size_t _file_offset, void* _buffer_ptr, size_t _offset, size_t _len) {
        GetCurrentSession()->Enqueue(_handle, _file_offset, _buffer_ptr, _offset, _len);
    }
    void VulkanStorage::Enqueue(FileHandle _handle, size_t _file_offset, void* _tex_ptr, EPixelFormat _format, uint3 _offset, uint3 _size, uint _mip_offset) {
        GetCurrentSession()->Enqueue(_handle, _file_offset, _tex_ptr, _format, _offset, _size, _mip_offset);
    }
    void VulkanStorage::Enqueue(void const* _mem, size_t _file_offset, void* _buffer_ptr, size_t _offset, size_t _len) {
        GetCurrentSession()->Enqueue(_mem, _file_offset, _buffer_ptr, _offset, _len);
    }
    void VulkanStorage::Enqueue(void const* _mem, size_t _file_offset, void* _tex_ptr, EPixelFormat _format, uint3 _offset, uint3 _size, uint _mip_offset) {
