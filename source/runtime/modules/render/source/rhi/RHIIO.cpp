...
namespace Moer::Render {
    ...
    public:
        ...
        static void EnqueueRequest(FileHandle _handle, size_t _offset, size_t _src_size, FileHandle _dst_handle, size_t _dst_offset, size_t _dst_size) {
    ...
    private:
        ...
        void InnerEnqueueRequest(FileHandle _handle, size_t _offset, size_t _src_size, FileHandle _in_dst_handle, size_t _dst_offset, size_t _dst_size) {
            std::lock_guard<std::mutex> lk(mutex);
            requests.push_back([=]() {
        ...
        void InnerEnqueueSignal(Event* _event_handle, uint64_t _timeline) {
            std::lock_guard<std::mutex> lk(mutex);
            requests.push_back([=, this]() { _event_handle->Signal(_timeline); });
        }
        void InnerWorkLoop() {
    ...
    private:
        void AsyncExecuteCmds(IOCommandListHolder& _cmd_holder) {
            ...
            auto exit_func = OnScopeExit([&]() {
                m_callbacks.push(
                    {std::move(callbacks), std::move(files), _cmd_holder.time_stamp});
            });
            ...
            for (auto& cmd : cmds) {
                ...
                std::visit(
                    [&](auto&& _src, auto&& _dst) {
                        if constexpr (std::is_same_v<std::decay_t<decltype(_src)>,
                                                     FileDesc>) {
                            ...
                            if constexpr (std::is_same_v<std::decay_t<decltype(_dst)>,
                                                         FileDesc>) {
                                IOLooper::EnqueueRequest(_src.data.data(), _src.data.size(), _dst.handle, _dst.offset);
                            } else {
    ...
    struct IOService::Impl {
        ...
        Impl() {
            IOLooper::Init();
            thread = MoerNew(std::jthread)([this]() { WorkLoop(); });
        }
        void Dispose() {
            requested_exit = true;
            MoerDelete(thread);
