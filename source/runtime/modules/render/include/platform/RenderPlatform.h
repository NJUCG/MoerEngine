...
namespace RHI {
    ...
    RHI_API class GenericRenderPlatformInfo {
        ...
        uint32_t b_support_rayTracing_indirect_instance_data : 1;// Whether instance transforms can be copied from the GPU to the TLAS instances buffer
        uint32_t b_support_highend_raytracing_reflections : 1;   // Whether fully-featured RT reflections can be used on the platform (with multi-bounce, translucency, etc.)
        uint32_t b_support_pathtracing : 1;                      // Whether real-time path tracer is supported on this platform (avoids compiling unnecessary shaders)
