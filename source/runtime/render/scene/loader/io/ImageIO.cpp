...
static void Generatemipmaps(ImageReadDesc& _desc, bool _is_generate_mipmaps) {
    if (_desc.mips > 1)
    ...
    // 如果需要生成mipmap，才会进入这个while
    while (_is_generate_mipmaps && (next_width > 1 || next_height > 1)) {
...
ImageReadDesc ImageIO::ReadFromFile(
    const std::filesystem::path& _path,
    uint32_t                     _desired_channal,
    EPixelFormat                 _fmt,
    bool                         _is_generate_mipmaps
) {
    ImageReadDesc desc;
...
ImageReadDesc ImageIO::ReadFromMemory(
