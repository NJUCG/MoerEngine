...
void MeshProcessor::GenerateMeshlets(const MeshProcessInput& input, MeshProcessOutput& output) {
    ...
    size_t target_vertex_count = meshopt_generateVertexRemap(
        &remap[0], input.index_data, origin_indices_count, input.vertex_data, origin_vertex_count, stride
    );

    Moer::Array<float> target_vertices(target_vertex_count * stride_float);

    meshopt_remapIndexBuffer(&target_indices[0], input.index_data, origin_indices_count, &remap[0]);

    meshopt_remapVertexBuffer(&target_vertices[0], input.vertex_data, origin_vertex_count, stride, &remap[0]);

    meshopt_optimizeVertexCache(
        &target_indices[0], target_indices.data(), origin_indices_count, target_vertex_count
    );
    ...
    for (uint32_t i = 0; i < meshlet_vertices.size(); i++) {
        auto* const copy_src =
            reinterpret_cast<float*>(target_vertices.data() + meshlet_vertices[i] * stride_float);
    ...
    std::for_each(meshlets.begin(), meshlets.end(), [&](const meshopt_Meshlet& m) {
        ...
        auto&& meshlet_bound = meshopt_computeMeshletBounds(
        ...
        meshlet_bounds[index].center = {
            meshlet_bound.center[0], meshlet_bound.center[1], meshlet_bound.center[2]
        };
...
MeshProcessOutput MeshProcessor::GenerateMeshlets(const MeshProcessInput& input) {
    ...
    output.meshlet_vertex_data = Moer::Array<float>(
        static_cast<float*>(input.vertex_data),
        static_cast<float*>(input.vertex_data) + input.vertex_count * input.vertex_stride / 4
    );
