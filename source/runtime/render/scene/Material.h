...
struct [[deprecated("ECS: MaterialComponent is not used yet")]] MaterialComponent {
    ...
    static_assert(
        SHADERTYPE_COUNT == sizeof(shader_type_defines) / sizeof(shader_type_defines[0]),
        "These values must match!"
    );

    uint8_t user_stencil_ref = 0;

    Moer::Vector4f base_color                 = Moer::Vector4f(1, 1, 1, 1);
    Moer::Vector4f specular_color             = Moer::Vector4f(1, 1, 1, 1);
    Moer::Vector4f emissive_color             = Moer::Vector4f(1, 1, 1, 0);
    Moer::Vector4f subsurface_scattering      = Moer::Vector4f(1, 1, 1, 0);
    Moer::Vector4f tex_mul_add                = Moer::Vector4f(1, 1, 0, 0);
    ...
    Moer::Vector4f sheen_color         = Moer::Vector4f(1, 1, 1, 1);
    float          sheen_roughness     = 0;
    float          clearcoat           = 0;
    float          clearcoat_roughness = 0;

    Moer::Vector2f tex_anim_direction    = Moer::Vector2f(0, 0);
    ...
    inline void SetSubsurfaceScatteringColor(Moer::Vector3f value) {
        SetDirty();
...
class MaterialBuilder {
    ...
    struct RENDER_API Parameter {
        Parameter(const std::string name_, ESamplerType type) :
            name(name_),
            samplerType(type),
            type(EParamaterType::SAMPLER) {}
        Parameter(const std::string name_, ETextureDimension type) :
            name(name_),
            textureType(type),
            type(EParamaterType::TEXTURE) {}
        Parameter(const std::string name_, ESamplerType samplerType, ETextureDimension textureType) :
            name(name_),
            samplerType(samplerType),
            textureType(textureType),
            type(EParamaterType::COMBINED) {}
        Parameter(const std::string name_, UniformType type, uint32_t size) :
            name(name_),
            uniformType(type),
            type(EParamaterType::UNIFORM),
            size(size) {}
        Parameter() = default;
...
public:
    ...
    RENDER_API MaterialBuilder&
    SetParameter(const std::string& name, UniformType type, uint32_t size) noexcept;
