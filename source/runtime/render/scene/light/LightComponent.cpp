...
CountableRef<LightComponent> LightComponent::ReadFromStream(InputStream& _stream) {
    ...
    if (type == ELightComponentType::DIRECTIONAL) {
        ...
        return MoerNew(DirectionalLightComponent)(color, intensity, direction, 0.f);

    } else if (type == ELightComponentType::POINT) {
        float3 position;
        _stream >> position;

        return MoerNew(PointLightComponent)(color, intensity, position);
        ...
        return MoerNew(SpotLightComponent)(
            color, intensity, position, direction, inner_cone_angle, outer_cone_angle
        );
        ...
        return MoerNew(EnvironmentLightComponent)(color, size);
...
Array<LightComponentRef> LightComponent::CreateDefaultLightComponents() {
    ...
    // Ambient Light
    {
        ...
        lights.push_back(
            MoerNew(AmbientLightComponent)(Vector3f(ambient_intensity, ambient_intensity, ambient_intensity))
        );
    }
