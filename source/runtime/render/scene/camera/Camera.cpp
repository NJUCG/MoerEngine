...
float Camera::GetTanHalfFov() const noexcept {
    return tan(m_fov_y / 180.f * HALF_PI);
}
...
Vector3f Camera::GetUp() const noexcept {
    return m_up;
}
Vector3f Camera::GetRight() const noexcept {
    return m_right;
}
Vector3f Camera::GetFront() const noexcept {
    return m_front;
}
Vector3f Camera::GetForward() const noexcept {
    return m_forward;
}
...
void Camera::SetWorldTransform(const Transform& _to_world_transform) noexcept {
    ...
    assert(
        Abs(to_world[3].x) < EPS && Abs(to_world[3].y) < EPS && Abs(to_world[3].z) < EPS &&
        Abs(to_world[3].w - 1.f) < EPS
    );
...
void Camera::UpdateVectors() {
    ...
    m_front = Vector3f(
        cos(Angle::DegreeToRadian(m_yaw)) * cos(Angle::DegreeToRadian(m_pitch)),
...
void Camera::UpdateProjectionMatrix() {
    ...
    m_projection_matrix =
        m_jittered_matrix *
        MakePerspectiveMatrixRH(Angle::DegreeToRadian(m_fov_y), m_aspect_ratio, m_far_clip, m_near_clip);
...
Camera Camera::CreateDefaultCamera() {
    ...
    Transform transform =
        Transform(Vector3f(0.0f, 0.0f, 0.0f), Vector3f(0.0f, 0.0f, 1.0f), Vector3f(0.0f, 1.0f, 0.0f));
...
std::string Camera::ToString() {
    ...
    str += "  position: (" + std::to_string(m_position[0]) + ", " + std::to_string(m_position[1]) + ", " +
    ...
    str += "  front: (" + std::to_string(m_front[0]) + ", " + std::to_string(m_front[1]) + ", " +
           std::to_string(m_front[2]) + ")\n";
    str += "  right: (" + std::to_string(m_right[0]) + ", " + std::to_string(m_right[1]) + ", " +
           std::to_string(m_right[2]) + ")\n";
    str += "  up: (" + std::to_string(m_up[0]) + ", " + std::to_string(m_up[1]) + ", " +
           std::to_string(m_up[2]) + ")\n";
    str += "  forward: (" + std::to_string(m_forward[0]) + ", " + std::to_string(m_forward[1]) + ", " +
           std::to_string(m_forward[2]) + ")\n";
    Vector4f planes[6];
    GetPlanes(planes);
    str += "  planes:\n";
    for (int i = 0; i < 6; i++) {
