...
struct RTInstance {
    ...
    uint GetMaterialType() const {
        return material_type_and_id & material_type_mask;
    }
    uint GetMaterialID() const {
        return material_type_and_id >> material_id_offset;
    }
    void SetMaterialType(uint _type) {
        material_type_and_id = (_type & material_type_mask) | (material_type_and_id & ~material_type_mask);
    }
    void SetMaterialID(uint _id) {
        material_type_and_id = (_id << material_id_offset) | (material_type_and_id & material_type_mask);
    }
    void SetMaterial(uint _type, uint _id) {
        material_type_and_id =
            (_type & material_type_mask) | ((_id << material_id_offset) & ~material_type_mask);
    }
};
struct Range {
    uint64 offset;
...
struct MeshBuffers {
    ...
    bool HasAttribute(EVertexAttributes _attr) const noexcept {
        return vertex_ranges[static_cast<size_t>(_attr)].size > 0;
    }
    Range GetAttributeRange(EVertexAttributes _attr) const noexcept {
        return vertex_ranges[static_cast<size_t>(_attr)];
    }
    Range& GetAttributeRange(EVertexAttributes _attr) noexcept {
        return vertex_ranges[static_cast<size_t>(_attr)];
    }
    ...
    void FillRanges() {
        ...
        for (size_t i = 0; i < vertex_factory_buffers.GetAttributesCount(); i++) {
            ...
            vertex_ranges[static_cast<size_t>(attr)] = {
                offset,
                vertex_factory_buffers.GetBufferLength(attr) * vertex_factory_buffers.GetSizeOfAttribute(attr)
            };
...
struct GpuScene {
    Render::BufferRef GetGpuBuffer(EGpuSceneResource _resource) const {
        return global_resources.buffers[(uint32_t)_resource];
    }
