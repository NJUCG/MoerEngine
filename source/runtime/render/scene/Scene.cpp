...
public:
    ...
    bool                    IsEntitiesEmpty() const noexcept {
        return m_entities.GetEntities().empty();
    }
    bool IsCamerasEmpty() const noexcept {
        return m_cameras.GetEntities().empty();
    }
    bool IsLightsEmpty() const noexcept {
        return m_lights.GetEntities().empty();
    }
    ...
    std::span<const Render::GeometryInstance> GetGeometryInstances() {
        return geom_instances;
    }
    std::span<const Render::BufferRef> GetIOPendingBuffers() const noexcept {
        return io_pending_buffers;
    }
    void ClearIOPendingBuffers() noexcept {
        io_pending_buffers.clear();
    }

    void SetCurrentEnvMap(EnvMapResource _env_map) {
        cur_env_map = _env_map;
    }
    EnvMapResource GetCurrentEnvMap() const {
        return cur_env_map;
    }
    void EmplaceIOImportedBuffer(Render::BufferRef _buffer) {
        io_pending_buffers.emplace_back(_buffer);
    }
...
void Scene::Impl::UpdateGpuData() {
    ...
    for (auto& entity : m_entities.GetEntities()) {
        ...
        for (auto& geo : info.geometries) {
            ...
            bitmask_to_mesh_buffers_map[buffers.vertex_factory_buffers.GetAttributesBitmask()] =
                geo->mesh_buffers;
            ...
            geo_data.normal_offset      = buffers.GetAttributeRange(EVertexAttributes::VA_NORMAL).offset +
                                     vtx_offset * VertexAttributesTool::GetSize(EVertexAttributes::VA_NORMAL);
            geo_data.tangent_offset =
                buffers.GetAttributeRange(EVertexAttributes::VA_TANGENT).offset +
                vtx_offset * VertexAttributesTool::GetSize(EVertexAttributes::VA_TANGENT);
            geo_data.texcoord0_offset =
                buffers.GetAttributeRange(EVertexAttributes::VA_TEXCOORD0).offset +
                vtx_offset * VertexAttributesTool::GetSize(EVertexAttributes::VA_TEXCOORD0);
            geo_data.texcoord1_offset =
                buffers.GetAttributeRange(EVertexAttributes::VA_TEXCOORD1).offset +
                vtx_offset * VertexAttributesTool::GetSize(EVertexAttributes::VA_TEXCOORD1);
            geo_data.mat_idx_and_type =
                geo->material_id << 8 | (uint)mat_instances[geo_idx]->GetMaterial()->GetType();
...

        {
            ...
            UnorderedMap<VertexAttributesBitmask, Array<Render::VertexBuffer>>& vtx_view =
                vtx_views[info.global_mesh_idx];
            UnorderedMap<VertexAttributesBitmask, Render::IndexBuffer>& idx_view =
                idx_views[info.global_mesh_idx];
            ...
            for (auto& [bitmask, mesh_buffers] : bitmask_to_mesh_buffers_map) {
                ...
                for (auto& attr : attrs) {
                    vtxs.push_back(
                        {mesh_buffers->vertex_buffer.Get(), mesh_buffers->GetAttributeRange(attr).offset}
                    );
                }
                vtx_view[bitmask] = std::move(vtxs);

                idx_view[bitmask] = {mesh_buffers->index_buffer->GetView(), EIndexElementType::IET_UINT32};
            }
        }
...
Scene::Impl::~Impl() noexcept {
    ...
    for (auto& entity : m_entities.GetEntities()) {
        ...
        for (auto& geo : info.geometries) {
            MeshBuffers& buffers =
                *geo->mesh_buffers; // 这里会重复访问同一个mesh_buffersss，重复清空，但是只会略微影响析构效率，问题不大
