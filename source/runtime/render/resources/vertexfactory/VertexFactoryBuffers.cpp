...
VertexFactoryBuffers::VertexFactoryBuffers(const Moer::Array<EVertexAttributes>& attributes) :
...
void VertexFactoryBuffers::AssignAllBuffers(
    const Array<std::tuple<EVertexAttributes, const void*, size_t>>& data
) {
...
void TestVertexFactoryBuffers() {
    ...
    VertexFactoryBuffers default_vf(
    ...
    VertexFactoryBuffers only_position_and_normal_vf(
        {EVertexAttributes::VA_POSITION, EVertexAttributes::VA_NORMAL}
    );
    ...
    assert(
        only_position_and_normal_vf.GetPixelFormatOfAttribute(EVertexAttributes::VA_POSITION) ==
        PF_R32G32B32_SFLOAT
    );
    assert(
        only_position_and_normal_vf.GetPixelFormatOfAttribute(EVertexAttributes::VA_NORMAL) == PF_R32_UINT
    );
    ...
    { // safe method to assign buffers
        ...
        Array<float3> only_position_data = {
            float3(1, 2, 3), float3(4, 5, 6), float3(7, 8, 9), float3(10, 11, 12)
        };
        Array<uint> only_normal_data = {1, 2, 3, 4};

        default_vf.AssignAllBuffers(
            {{EVertexAttributes::VA_POSITION, default_position_data.data(), default_position_data.size()},
        ...
        only_position_and_normal_vf.AssignAllBuffers(
            {{EVertexAttributes::VA_POSITION, only_position_data.data(), only_position_data.size()},
        ...
        for (size_t i = 0; i < only_position_and_normal_vf.GetAttributesCount(); i++) {
            assert(
                only_position_and_normal_vf.GetBufferLength(only_position_and_normal_vf.GetAttribute(i)) ==
                only_position_data.size()
            );
        }
        ...
        auto only_position_buffer =
            reinterpret_cast<float3*>(only_position_and_normal_vf.GetBufferData(EVertexAttributes::VA_POSITION
            ));
        auto only_position_buffer_length =
            only_position_and_normal_vf.GetBufferLength(EVertexAttributes::VA_POSITION);

        auto only_normal_buffer =
            reinterpret_cast<uint*>(only_position_and_normal_vf.GetBufferData(EVertexAttributes::VA_NORMAL));
        auto only_normal_buffer_length =
            only_position_and_normal_vf.GetBufferLength(EVertexAttributes::VA_NORMAL);
    ...
    { // unsafe method to assign buffers
        ...
        auto* tangent_buffer =
            reinterpret_cast<TTangent*>(default_vf.GetBufferData(EVertexAttributes::VA_TANGENT));
        auto* normal_buffer =
            reinterpret_cast<TNormal*>(default_vf.GetBufferData(EVertexAttributes::VA_NORMAL));
        ...
        memcpy(
            position_buffer,
            default_position_data.data(),
            default_position_data.size() * default_vf.GetSizeOfAttribute(EVertexAttributes::VA_POSITION)
        );
        memcpy(
            tangent_buffer,
            default_tangent_data.data(),
            default_tangent_data.size() * default_vf.GetSizeOfAttribute(EVertexAttributes::VA_TANGENT)
        );
        memcpy(
            normal_buffer,
            default_normal_data.data(),
            default_normal_data.size() * default_vf.GetSizeOfAttribute(EVertexAttributes::VA_NORMAL)
        );
        memcpy(
            texcoord_buffer,
            default_texcoord_data.data(),
            default_texcoord_data.size() * default_vf.GetSizeOfAttribute(EVertexAttributes::VA_TEXCOORD0)
        );
    ...
    { // input stream & output stream test
        ...
        for (size_t i = 0; i < default_vf.GetAttributesCount(); i++) {
            ...
            assert(
                default_vf.GetBufferLength(default_vf.GetAttribute(i)) ==
                new_vf.GetBufferLength(new_vf.GetAttribute(i))
            );
            assert(
                default_vf.GetSizeOfAttribute(default_vf.GetAttribute(i)) ==
                new_vf.GetSizeOfAttribute(new_vf.GetAttribute(i))
            );
            assert(
                default_vf.GetPixelFormatOfAttribute(default_vf.GetAttribute(i)) ==
                new_vf.GetPixelFormatOfAttribute(new_vf.GetAttribute(i))
            );
        }
        ...
        auto position_buf_size = default_vf.GetBufferLength(EVertexAttributes::VA_POSITION) *
                                 default_vf.GetSizeOfAttribute(EVertexAttributes::VA_POSITION);
        for (size_t i = 0; i < position_buf_size; i++) {
            assert(default_position_buffer[i] == new_position_buffer[i]);
        }

        auto tangent_buf_size = default_vf.GetBufferLength(EVertexAttributes::VA_TANGENT) *
                                default_vf.GetSizeOfAttribute(EVertexAttributes::VA_TANGENT);
        for (size_t i = 0; i < tangent_buf_size; i++) {
            assert(default_tangent_buffer[i] == new_tangent_buffer[i]);
        }

        auto normal_buf_size = default_vf.GetBufferLength(EVertexAttributes::VA_NORMAL) *
                               default_vf.GetSizeOfAttribute(EVertexAttributes::VA_NORMAL);
        for (size_t i = 0; i < normal_buf_size; i++) {
            assert(default_normal_buffer[i] == new_normal_buffer[i]);
        }

        auto texcoord_buf_size = default_vf.GetBufferLength(EVertexAttributes::VA_TEXCOORD0) *
                                 default_vf.GetSizeOfAttribute(EVertexAttributes::VA_TEXCOORD0);
