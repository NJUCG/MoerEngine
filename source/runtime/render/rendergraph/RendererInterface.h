...
public:
    ...
    std::string GenerateInfoString() const {
        ...
        return "(" + type + array_suffix + " " + std::to_string(Extent.x) + "x" + std::to_string(Extent.y) +
               " PF_" + std::to_string(static_cast<uint32>(Format)) + samples_suffix + ")";
    }
...
/**
 * Render thread side, use CountableRef<IPooledRenderTarget>, allows sharing and VisualizeTexture
 */
struct FSceneRenderTargetItem {
    TextureRef TargetableTexture;
};

/** Pure virtual interface for pooled render targets.
 *  Extracted from RenderTargetPool.h to break circular includes:
 *  RenderGraphResource.h needs IPooledRenderTarget (for CountableRef member),
 *  RenderTargetPool.h needs FRDGPooledTexture (for FPooledRenderTarget value member). */
struct IPooledRenderTarget {
    ...
    // Refcounting — AddRef/Release/GetRefCount for external use.
    // DeRef() is an alias for Release() to satisfy CountableRef<IPooledRenderTarget>.
