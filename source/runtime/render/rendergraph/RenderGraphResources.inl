...
inline ERHIPipeline FRDGSubresourceState::GetPipelines() const {
    ...
    Pipelines |=
        FirstPass[ERHIPipeline::AsyncCompute].IsValid() ? ERHIPipeline::AsyncCompute : ERHIPipeline::None;
    return Pipelines;
}
...
inline FRHIBufferCreateInfo Translate(const FRDGBufferDesc& InDesc) {
    ...
    if (EnumHasAnyFlag(InDesc.Usage, EBufferUsageFlags::VertexBuffer)) {
        CreateInfo.Stride = 0;
        CreateInfo.Usage  = InDesc.Usage | BUF_VertexBuffer;
    } else if (EnumHasAnyFlag(InDesc.Usage, EBufferUsageFlags::StructuredBuffer)) {
...
inline FRDGTextureDesc Translate(const FPooledRenderTargetDesc& InDesc) {
    ...
    if (InDesc.Depth > 0) {
        ...
        OutDesc.Dimension =
            InDesc.bIsArray ? ETextureDimension::TextureCubeArray : ETextureDimension::TextureCube;
...
//     if (Desc.MetaData == ERDGTextureMetaDataAccess::None && Desc.Texture &&
//         Desc.Texture->Desc.Format == PF_DepthStencil) {
//         // PF_X24_G8 is used to indicate that this is a view on the stencil plane. Otherwise, it is a view on the depth plane
//         Range.PlaneSlice     = Desc.Format == PF_X24_G8 ? FRHITransitionInfo::kStencilPlaneSlice :
//                                                           FRHITransitionInfo::kDepthPlaneSlice;
...
inline FRDGBufferSRVDesc::FRDGBufferSRVDesc(FRDGBufferRef InBuffer) : Buffer(InBuffer) {
    if (EnumHasAnyFlag(Buffer->Desc.Usage, EBufferUsageFlags::DrawIndirect)) {
        Format = PF_R32_UINT;
    } else if (EnumHasAnyFlag(Buffer->Desc.Usage, EBufferUsageFlags::AccelerationStructure)) {
        // nothing special here
    }
}

inline FRDGBufferUAVDesc::FRDGBufferUAVDesc(FRDGBufferRef InBuffer) : Buffer(InBuffer) {
    if (EnumHasAnyFlag(Buffer->Desc.Usage, EBufferUsageFlags::DrawIndirect)) {
        Format = PF_R32_UINT;
    }
}
...
//             EnumHasAnyFlag(DepthTexture->Desc.Flags, TexCreate_Memoryless) ?
...
//         }
//         RenderTargetsInfo.DepthTargetStoreAction   = RenderTargetsInfo.DepthStencilAccess.IsUsingDepth() ?
//                                                          StoreAction :
//                                                          ERenderTargetStoreAction::ENoAction;
//         RenderTargetsInfo.StencilTargetStoreAction = RenderTargetsInfo.DepthStencilAccess.IsUsingStencil() ?
//                                                          StoreAction :
//                                                          ERenderTargetStoreAction::ENoAction;
//     } else {
