...
VertexFactory::VertexFactory(VertexAttributesBitmask _flags, bool _is_shadow_depth_pass) :
    mask(_flags),
...
Shader& VertexShader::GetShader(Moer::Render::VertexFactory* _factory) {
    ...
    return shader_map
        .emplace(
            *_factory,
            ShaderManager::Get().CompileVertexShader(
                shader_path, _factory, entry_name, src_environment, mutation_id
            )
        )
        .first->second;
}
