...
namespace Moer::Render {
    struct PsoMapKey {
        ...
        PsoMapKey(VertexAttributesBitmask bitmask, bool is_shadow_depth_pass)
            : bitmask(bitmask), is_shadow_depth_pass(is_shadow_depth_pass) {}
...
namespace std {
    ...
    struct hash<Moer::Render::PsoMapKey> {
        size_t operator()(const Moer::Render::PsoMapKey& key) const {
            return std::hash<Moer::VertexAttributesBitmask>()(key.bitmask) ^ std::hash<bool>()(key.is_shadow_depth_pass);
        }
    };
}// namespace std
