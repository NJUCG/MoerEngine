...
namespace Moer::Render {
    ...
    struct IOCmd {
        ...
        uint64 SizeByte() const {
            return std::visit(
                Overload{
                    [](const RawDataDesc& _src) { return _src.data.size_bytes(); },
                    [](const BufferViewDesc& _src) { return uint64(_src.size); },
                    [](const TextureViewDesc& _src) { return _src.GetByteSize(); }},
                dst);
        }
    };
    ...
    public:
        ...
        void ImportResources(EQueueType _src_queue, const Array<ImportTexture>& _textures, const Array<ImportBuffer>& _buffers);
