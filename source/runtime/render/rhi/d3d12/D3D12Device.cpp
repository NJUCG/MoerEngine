...
Moer::Render::D3D12Device::D3D12Device(const D3D12RHIConfig&& _config) : config(_config) {
    ...
    if (SUCCEEDED(debug_interface.As(&debug_info_queue1))) { // always failed because of not support ???
        ...
        DX_CHECK_HRESULT(debug_info_queue1->RegisterMessageCallback(
            debug_log, D3D12_MESSAGE_CALLBACK_FLAG_NONE, nullptr, &debug_log_callback_cookie
        ));
    ...
    ASSERT(
        feature_supports.ResourceBindingTier() >= D3D12_RESOURCE_BINDING_TIER_3
    );                                                                     // ResourceDescriptorHeap[index]
    ASSERT(feature_supports.HighestShaderModel() >= D3D_SHADER_MODEL_6_6); // ResourceDescriptorHeap[index]
    ASSERT(
        feature_supports.RaytracingTier() >= D3D12_RAYTRACING_TIER_1_1
    ); // 1.0 for basic dxr, 1.1 for rayquery
    ...
    csu_heap_cpu         = MakeUnique<D3D12CpuDescriptorAllocator>(
        this, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, feature_supports.MaxViewDescriptorHeapSize()
    ); // ~1e6
    rtv_heap_cpu = MakeUnique<D3D12CpuDescriptorAllocator>(
        this, D3D12_DESCRIPTOR_HEAP_TYPE_RTV, 256
    ); // just some small number, should be enough
    dsv_heap_cpu     = MakeUnique<D3D12CpuDescriptorAllocator>(this, D3D12_DESCRIPTOR_HEAP_TYPE_DSV, 256);
    sampler_heap_cpu = MakeUnique<D3D12CpuDescriptorAllocator>(
        this,
        D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER,
        feature_supports.MaxSamplerDescriptorHeapSizeWithStaticSamplers()
    ); // ~2048
    csu_heap_gpu = MakeUnique<D3D12GpuDescriptorAllocator>(
        this, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, feature_supports.MaxViewDescriptorHeapSize() / 2, 4096
    ); // ~1e6
    sampler_heap_gpu = MakeUnique<D3D12GpuDescriptorAllocator>(
        this,
        D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER,
        feature_supports.MaxSamplerDescriptorHeapSizeWithStaticSamplers() / 2,
        256
    ); // ~2048
}
...
CheckReflectionTypeMatch(const ReflectParamInfo::Dxil& resource_info, const ShaderArgCppInfo& arg_info) {
    ...
    if (arg_info.type == EShaderArgType::SDA_Buffer)
        return IsShaderVarCommonBuffer(resource_info) || IsShaderVarConstantBuffer(resource_info);
    if (arg_info.type == EShaderArgType::SDA_Texture)
        return IsShaderVarTexture(resource_info);
    if (arg_info.type == EShaderArgType::SDA_Sampler)
        return IsShaderVarSampler(resource_info);
    if (arg_info.type == EShaderArgType::SDA_TLAS)
        return IsShaderVarRaytracingAccelerationStructure(resource_info);
    return false;
};
...
void D3D12PipelineState::PipelineLayout::Add(
    uint8                         _idx_in_cpp_args,
    ...
    if (_b_special_bindless) {
        the_bindless_array.emplace(
            _idx_in_cpp_args, uint8(-1), uint8(_resource_info.slot), uint8(_resource_info.space)
        );
    } else if (IsShaderVarRootConstant(_resource_info)) {
        root_constant.emplace(
            _idx_in_cpp_args,
            uint8(-1),
            _resource_info.byte_size,
            uint8(_resource_info.slot),
        ...
        if (IsShaderVarConstantBuffer(_resource_info)) {
            ...
            srv_table->entries.emplace_back(
                _idx_in_cpp_args,
                uint8(srv_table->entries.size()),
                uint8(bind_count),
                uint8(_resource_info.slot),
                uint8(_resource_info.space)
            );
            ...
            uav_table->entries.emplace_back(
                _idx_in_cpp_args,
                uint8(uav_table->entries.size()),
                uint8(bind_count),
                uint8(_resource_info.slot),
                uint8(_resource_info.space)
            );
            ...
            sampler_table->entries.emplace_back(
                _idx_in_cpp_args,
                uint8(sampler_table->entries.size()),
                uint8(bind_count),
                uint8(_resource_info.slot),
                uint8(_resource_info.space)
            );
...
void D3D12PipelineState::BuildRootSignature(const PipelineLayout& _layout) {
...

    {
        ...
        static constexpr D3D12_DESCRIPTOR_RANGE_FLAGS kSrvDescriptorRangeFlags =
            D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE |
            D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE;
        static constexpr D3D12_DESCRIPTOR_RANGE_FLAGS kCbvDescriptorRangeFlags =
            D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE |
            D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE;
        static constexpr D3D12_DESCRIPTOR_RANGE_FLAGS kUavDescriptorRangeFlags =
            D3D12_DESCRIPTOR_RANGE_FLAG_DATA_VOLATILE | D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE;
        static constexpr D3D12_DESCRIPTOR_RANGE_FLAGS kSamplerDescriptorRangeFlags =
            D3D12_DESCRIPTOR_RANGE_FLAG_DESCRIPTORS_VOLATILE;
        static constexpr D3D12_ROOT_DESCRIPTOR_FLAGS kCbvRootDescriptorFlags =
        ...
        if (layout.root_constant) {
            root_parameters[layout.root_constant->idx_in_root_sig].InitAsConstants(
                layout.root_constant->byte_size / 4,
                layout.root_constant->slot,
                layout.root_constant->space,
                D3D12_SHADER_VISIBILITY_ALL
            );
        }
        if (layout.the_bindless_array) {
            root_parameters[layout.the_bindless_array->idx_in_root_sig].InitAsShaderResourceView(
                layout.the_bindless_array->slot,
                layout.the_bindless_array->space,
                D3D12_ROOT_DESCRIPTOR_FLAG_DATA_STATIC_WHILE_SET_AT_EXECUTE,
                D3D12_SHADER_VISIBILITY_ALL
            );
        }
        if (layout.root_cbvs) {
            for (const auto& cbv : *layout.root_cbvs) {
                root_parameters[cbv.idx_in_root_sig].InitAsConstantBufferView(
                    cbv.slot, cbv.space, kCbvRootDescriptorFlags, D3D12_SHADER_VISIBILITY_ALL
                );
            }
        }
        uint range_offset = 0;
        if (layout.srv_table) {
            for (const auto& e : layout.srv_table->entries) {
                descriptor_ranges[range_offset + e.idx_in_table].Init(
                    D3D12_DESCRIPTOR_RANGE_TYPE_SRV, e.bind_count, e.slot, e.space, kSrvDescriptorRangeFlags
                ); // one table can have different type ranges. we now do like this for simplicity
            }
            root_parameters[layout.srv_table->idx_in_root_sig].InitAsDescriptorTable(
                layout.srv_table->entries.size(),
                descriptor_ranges.data() + range_offset,
                D3D12_SHADER_VISIBILITY_ALL
            );
        ...
        if (layout.uav_table) {
            for (const auto& e : layout.uav_table->entries) {
                descriptor_ranges[range_offset + e.idx_in_table].Init(
                    D3D12_DESCRIPTOR_RANGE_TYPE_UAV, e.bind_count, e.slot, e.space, kUavDescriptorRangeFlags
                );
            }
            root_parameters[layout.uav_table->idx_in_root_sig].InitAsDescriptorTable(
                layout.uav_table->entries.size(),
                descriptor_ranges.data() + range_offset,
                D3D12_SHADER_VISIBILITY_ALL
            );
        ...
        if (layout.sampler_table) {
            for (const auto& e : layout.sampler_table->entries) {
                descriptor_ranges[range_offset + e.idx_in_table].Init(
                    ...
                    e.slot,
                    e.space,
                    kSamplerDescriptorRangeFlags
                );
            }
            root_parameters[layout.sampler_table->idx_in_root_sig].InitAsDescriptorTable(
                layout.sampler_table->entries.size(),
                descriptor_ranges.data() + range_offset,
                D3D12_SHADER_VISIBILITY_ALL
            );
    ...
    D3D12_ROOT_SIGNATURE_FLAGS root_sig_flags = D3D12_ROOT_SIGNATURE_FLAG_CBV_SRV_UAV_HEAP_DIRECTLY_INDEXED |
                                                D3D12_ROOT_SIGNATURE_FLAG_SAMPLER_HEAP_DIRECTLY_INDEXED;

    CD3DX12_VERSIONED_ROOT_SIGNATURE_DESC root_sig_desc;
    root_sig_desc.Init_1_1(
        root_parameters.size(), root_parameters.data(), 0, nullptr, root_sig_flags
    ); //todo static samplers

    D3D12_FEATURE_DATA_ROOT_SIGNATURE feature_data{
        .HighestVersion = device->GetFeatureSupport().HighestRootSignatureVersion()
    };
    ComPtr<ID3DBlob> signature_blob;
    ComPtr<ID3DBlob> error;
    DX_CHECK_HRESULT(D3DX12SerializeVersionedRootSignature(
        &root_sig_desc, feature_data.HighestVersion, signature_blob.GetAddressOf(), error.GetAddressOf()
    ));
    DX_CHECK_HRESULT(device->Native()->CreateRootSignature(
        0,
...
PipelineHandle D3D12Device::CreatePipeline(PipelineShaderInfo&& _shaders) {
    ...
    DX_CHECK_HRESULT(device->CreateComputePipelineState(
        &pso_desc, IID_PPV_ARGS(pso->pipeline_state.ReleaseAndGetAddressOf())
    ));
...
CopyQueue& D3D12Device::GetCopyQueue() {
    ...
    struct DummyCopyQueue : CopyQueue {
        IOWaitEvt Execute(IOSubmission&& _submit) {
            return {};
        }
        virtual IOWaitEvt Execute(CmdSubmit&& _submit) {
            return {};
        }
        virtual IOWaitEvt Execute(IOQueueSubmission&& _submit) {
            return {};
        }
        ...
        virtual FenceRef GetFenceHandle() {
            return nullptr;
        };
    ...
    *ppAdapter = nullptr;
    ...
    if (adapter.Get() == nullptr) {
        for (UINT adapterIndex = 0;
             SUCCEEDED(pFactory->EnumAdapters1(adapterIndex, adapter.ReleaseAndGetAddressOf()));
             ++adapterIndex) {
            DXGI_ADAPTER_DESC1 desc;
            adapter->GetDesc1(&desc);
...
D3D12Device::PushCsuDescriptor(std::span<const DescriptorIndex> _index_in_cpu_heap) {
    ...
    for (uint i = 0; i < count; ++i) {
        device->CopyDescriptorsSimple(
            1,
            csu_heap_gpu->GetOffsetHandleCpu({start.index + i}),
            csu_heap_cpu->GetOffsetHandleCpu(_index_in_cpu_heap[i]),
...
D3D12_GPU_DESCRIPTOR_HANDLE
D3D12Device::PushSamplerDescriptor(std::span<const DescriptorIndex> _index_in_cpu_heap) {
    const uint            count = _index_in_cpu_heap.size();
    const DescriptorIndex start = sampler_heap_gpu->Allocate(count);
    for (uint i = 0; i < count; ++i) {
        device->CopyDescriptorsSimple(
            1,
            sampler_heap_gpu->GetOffsetHandleCpu({start.index + i}),
            sampler_heap_cpu->GetOffsetHandleCpu(_index_in_cpu_heap[i]),
            D3D12_DESCRIPTOR_HEAP_TYPE_SAMPLER
        );
    }
...
D3D12Buffer::D3D12Buffer(D3D12Device* _device, const BufferInfo& _info) :
    Buffer(_info),
    ...
    if (EnumHasAnyFlag(
            _info.usage,
            EBufferUsageFlags::UNORDERED_ACCESS | EBufferUsageFlags::ACCELERATION_STRUCTURE_SCRATCH
        )) {
        resourceDesc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
    }
    if (EnumHasAnyFlag(
            _info.usage,
            EBufferUsageFlags::ACCELERATION_STRUCTURE | EBufferUsageFlags::ACCELERATION_STRUCTURE_BUILD_INPUT
        )) {
        resourceDesc.Flags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
...
D3D12Buffer::D3D12Buffer(D3D12Device* _device, const BufferInfo& _info, Allocation&& _allocation) :
    Buffer(_info),
    D3D12DeviceChild(_device),
...
DescriptorIndex D3D12Buffer::CreateSrv(const BufferView& _range, ED3D12ShaderVariableType _type) {
    ...
    const uint byte_stride =
        _type == ED3D12ShaderVariableType::ByteAddressBuffer ?
            4 :
            (_type == ED3D12ShaderVariableType::StructuredBuffer ?
                 GetStride() :
                 D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::GetBitsPerUnit(viewDesc.format) / 8);
    ...
    srvDesc.Format =
        _type == ED3D12ShaderVariableType::ByteAddressBuffer ?
            ...
            (_type == ED3D12ShaderVariableType::StructuredBuffer ? DXGI_FORMAT_UNKNOWN : viewDesc.format);
    ...
    srvDesc.Buffer.StructureByteStride =
        _type == ED3D12ShaderVariableType::StructuredBuffer ? byte_stride : 0;
    srvDesc.Buffer.Flags = _type == ED3D12ShaderVariableType::ByteAddressBuffer ? D3D12_BUFFER_SRV_FLAG_RAW :
                                                                                  D3D12_BUFFER_SRV_FLAG_NONE;

    auto index = device->GetCsuHeap()->Allocate();
    device->Native()->CreateShaderResourceView(
        allocation.resource, &srvDesc, device->GetCsuHeap()->GetOffsetHandleCpu(index)
    );
...
DescriptorIndex D3D12Buffer::CreateUav(const BufferView& _range, ED3D12ShaderVariableType _type) {
    ...
    const uint byte_stride =
        _type == ED3D12ShaderVariableType::RWByteAddressBuffer ?
            4 :
            (_type == ED3D12ShaderVariableType::RWStructuredBuffer ?
                 GetStride() :
                 D3D12_PROPERTY_LAYOUT_FORMAT_TABLE::GetBitsPerUnit(viewDesc.format) / 8);
    ...
    uavDesc.Format =
        _type == ED3D12ShaderVariableType::RWByteAddressBuffer ?
            DXGI_FORMAT_R32_TYPELESS :
            (_type == ED3D12ShaderVariableType::RWStructuredBuffer ? DXGI_FORMAT_UNKNOWN : viewDesc.format);
    ASSERT(_range.GetByteOffset() % byte_stride == 0);
    ...
    uavDesc.Buffer.StructureByteStride =
        _type == ED3D12ShaderVariableType::RWStructuredBuffer ? byte_stride : 0;
    uavDesc.Buffer.Flags                = _type == ED3D12ShaderVariableType::RWByteAddressBuffer ?
    ...
    device->Native()->CreateUnorderedAccessView(
        allocation.resource, nullptr, &uavDesc, device->GetCsuHeap()->GetOffsetHandleCpu(index)
    );
    srv_uav_views.emplace_back(viewDesc, index);
    return index;
}
...
Allocation D3D12GpuGlobalAllocator::AllocateTextureHeap(
    ...
    desc.Flags          = D3D12MA::ALLOCATION_FLAGS::ALLOCATION_FLAG_STRATEGY_BEST_FIT;
    desc.ExtraHeapFlags = _is_rtv_dsv ? D3D12_HEAP_FLAG_ALLOW_ONLY_RT_DS_TEXTURES :
...
struct D3D12CommandPreprocessVisitor {
    ...
    D3D12CommandPreprocessVisitor(D3D12CommandResourceAllocator& allocator) :
        allocator(allocator),
        tracker(allocator.GetResourceStateTracker()) {}

    void Visit(const UploadBufferCmd& _cmd) {
        D3D12Buffer* dst_buffer = reinterpret_cast<D3D12Buffer*>(_cmd.Handle());
        tracker.RecordState(
            dst_buffer, {.sync = D3D12_BARRIER_SYNC_COPY, .access = D3D12_BARRIER_ACCESS_COPY_DEST}
        );
    }
    ...
    void Visit(const CopyBackBufferCmd& _cmd) {
        ...
        tracker.RecordState(
            src_buffer, {.sync = D3D12_BARRIER_SYNC_COPY, .access = D3D12_BARRIER_ACCESS_COPY_SOURCE}
        );
    }
        ...
        Variant(_arg).Match(
            ...
            [&](const BufferView& arg) {
                ...
                tracker.RecordState(
                    ...
                    sync_flag == D3D12_BARRIER_SYNC_COMPUTE_SHADING ? EPassType::Compute :
                                                                      EPassType::Graphics, // a bit strange...
                    b_write
                );
            },
    ...
    void Visit(const DispatchCmd& _cmd) {
        ...
        auto func = [&](const TArg& _arg, uint _idx) {
            VisitArgs(
                _arg, pipeline.binding_infos[_idx].state_flags, pipeline.binding_infos[_idx].pipeline_flags
            );
        };
...
struct D3D12CommandVisitor {
    ...
    D3D12CommandVisitor(D3D12CommandResourceAllocator& allocator) :
        allocator(allocator),
        tracker(allocator.GetResourceStateTracker()),
        cmd_list(*allocator.GetCommandList()) {}
    ...
    void Visit(const UploadBufferCmd& _cmd) {
        ...
        cmd_list.CopyBuffer(
            tmp_buffer.buffer, buffer, _cmd.ByteSize(), tmp_buffer.byte_offset, _cmd.Offset()
        );
    }
    ...
    void Visit(const CopyBufferToTextureCmd& _cmd) {
        ...
        cmd_list.CopyBufferToTexture(
            src_buffer, dst_texture, _cmd.SrcOffset(), _cmd.DstOffset(), _cmd.Size(), _cmd.MipLevel()
        );
    }
    ...
    void Visit(const CopyTextureToBufferCmd& _cmd) {
        ...
        cmd_list.CopyTextureToBuffer(
            src_texture, dst_buffer, _cmd.SrcOffset(), _cmd.DstOffset(), _cmd.Size(), _cmd.MipLevel()
        );
    }
    ...
    void Visit(const UploadTextureCmd& _cmd) {
        ...
        cmd_list.CopyBufferToTexture(
            tmp_buffer.buffer, texture, tmp_buffer.byte_offset, _cmd.Offset(), _cmd.Size(), _cmd.MipLevel()
        );
    }
...
D3D12Fence::D3D12Fence(D3D12Device* _device) : D3D12DeviceChild(_device) {
    DX_CHECK_HRESULT(
        device->Native()->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(fence.ReleaseAndGetAddressOf()))
    );
...
D3D12CommandResourceAllocator::D3D12CommandResourceAllocator(D3D12GraphicsCommandQueue& _queue) :
    ...
    DX_CHECK_HRESULT(device->Native()->CreateCommandAllocator(
        queue.GetType(), IID_PPV_ARGS(cmd_allocator.ReleaseAndGetAddressOf())
    ));
...
CreateLargeStagingBuffer(D3D12Device* _device, D3D12_HEAP_TYPE _heap_type, uint64 _byte_size) {
    ...
    return MakeUnique<D3D12Buffer>(
        _device, BufferInfo{_byte_size, 1, EBufferUsageFlags::CPU_VISIBLE}, std::move(allocation)
    );
}
...
D3D12StagingBufferView
D3D12CommandResourceAllocator::AllocateReadbackBuffer(uint64 _size, uint32 _alignment) {
    if (_size < larget_buffer_byte_size_threshold) {
...
D3D12CommandList::D3D12CommandList(D3D12CommandResourceAllocator& allocator) : allocator(allocator) {
    DX_CHECK_HRESULT(allocator.device->Native()->CreateCommandList1(
        0,
        allocator.queue.GetType(),
...
void D3D12CommandList::BindDescriptors(const PipelineHandle& _pso_handle, const ArrayArguments& _args) {
    ...
    if (layout.root_constant) {
        ASSERT(
            layout.root_constant->idx_in_cpp_args < _args.Size() &&
            layout.root_constant->idx_in_cpp_args < _pso_handle.binding_infos.size()
        );
        ...
        list->SetComputeRoot32BitConstants(
            layout.root_constant->idx_in_root_sig, constants.size(), constants.data(), 0
        );
    }
    if (layout.root_cbvs) {
        for (const auto& cbv : *layout.root_cbvs) {
            ASSERT(
                cbv.idx_in_cpp_args < _args.Size() && cbv.idx_in_cpp_args < _pso_handle.binding_infos.size()
            );
            ...
            list->SetComputeRootConstantBufferView(
                cbv.idx_in_root_sig, buf->GetGpuVirtualAddress() + buf_view.GetByteOffset()
            ); // device-side buffer
        }
    }
    if (layout.the_bindless_array) {
    ...
    if (layout.srv_table) {
        ...
        list->SetComputeRootDescriptorTable(
            layout.srv_table->idx_in_root_sig, allocator.GetDevice()->PushCsuDescriptor(indices)
        );
    }
    if (layout.uav_table) { // todo unify
        ...
        list->SetComputeRootDescriptorTable(
            layout.uav_table->idx_in_root_sig, allocator.GetDevice()->PushCsuDescriptor(indices)
        );
    }
    if (layout.sampler_table) {
        FATAL("not implemented");
...
void D3D12CommandList::CopyBuffer(
    ...
    list->CopyBufferRegion(_dst->Native(), _dst_offset, _src->Native(), _src_offset, _size);
}
...
void D3D12CommandList::CopyBufferToTexture(
    ...
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT layout;
    {
        ...
        uint32     x;
        allocator.GetDevice()->Native()->GetCopyableFootprints(
            &desc, dst.SubresourceIndex, 1, _src_offset, &layout, nullptr, nullptr, &required_size
        );
    ...
    list->CopyTextureRegion(&dst, _dst_offset.x, _dst_offset.y, _dst_offset.z, &src, nullptr);
}
void D3D12CommandList::CopyTextureToBuffer(
    ...
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT footprint;
    {
        ...
        allocator.GetDevice()->Native()->GetCopyableFootprints(
            &desc, src.SubresourceIndex, 1, _dst_offset, &footprint, nullptr, nullptr, &required_size
        );
...
void D3D12CommandList::CopyTexture(
    ...
    list->CopyTextureRegion(&dst, _dst_offset.x, _dst_offset.y, _dst_offset.z, &src, &srcBox);
}
D3D12Buffer*
D3D12CommandResourceAllocator::HeapPlacedBufferAllocator::Allocate(uint64 _size, EBufferUsageFlags _usage) {
...
void D3D12BuddyAllocator::DeallocateBlock(uint32 _offset, uint32 _order) {
    ...
    if (auto it = free_blocks[_order].find(buddy_id);
        it != free_blocks[_order].end()) // If buddy block is free, merge it
    {
...
D3D12MultiBuddyAllocatorAutoFree::D3D12MultiBuddyAllocatorAutoFree(
    D3D12Device*    _device,
    D3D12_HEAP_TYPE _heap_type,
    uint32          _each_total_byte_size,
    uint32          _min_block_byte_size
    ...
    D3D12DeviceChild(_device),
    allocator(_device, _heap_type, _each_total_byte_size, _min_block_byte_size) {}
...
D3D12FastConstantAllocator::D3D12FastConstantAllocator(
    D3D12Device* _device,
    uint32       _total_byte_size,
    uint32       _min_block_byte_size
) :
    D3D12DeviceChild(_device),
    allocator(_device, D3D12_HEAP_TYPE_UPLOAD, _total_byte_size, _min_block_byte_size),
...
D3D12StagingBufferView D3D12FastConstantAllocator::Allocate(uint32 _size) {
    ...
    if (offset + aligned_size > page_size) {
        ...
        ASSERT(
            underlying_current.gpu_base_address_remapped == underlying_current.buffer->GetGpuVirtualAddress()
        );
    }
...
D3D12Texture::D3D12Texture(D3D12Device* _device, const TextureInfo& _info) :
    D3D12DeviceChild(_device),
    Texture(_info) {
    ...
    if (resourceDesc.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL) {
        ASSERT(EnumHasAnyFlag(
            GetAspectFlags(), ETextureAspectFlags::DEPTH_SLICE | ETextureAspectFlags::STENCIL_SLICE
        ));
...
D3D12Texture::D3D12Texture(D3D12Device* _device, const TextureInfo& _info, Allocation&& _allocation) :
    D3D12DeviceChild(_device),
    Texture(_info),
    allocation(std::move(_allocation)) {}
...
D3D12_RESOURCE_FLAGS METoDxTextureResourceFlags(ETextureUsageFlags _me_flags) {
    ...
    ASSERT(
        false == EnumHasAnyFlag(_me_flags, ETextureUsageFlags::CPU_VISIBLE)
    ); // won't create texture directly for upload/readback
...
void D3D12ResourceStateTracker::RecordState(
    ...
    switch (_state) {
        ...
        case ETextureState::SAMPLE:
            ...
            desc = {
                ...
                .sync   = _pass_type == EPassType::Graphics ? D3D12_BARRIER_SYNC_DRAW :
        ...
        case ETextureState::UNORDERED_ACCESS:
            DASSERT(_is_write);
            DASSERT(
                _pass_type != EPassType::Copy
            ); // NOTE there is a special case for clearUAV commands, buffer is viewed as UAV, but it works like a copy/transfer
            desc = {
        ...
        default:
            FATAL(
                "Unsupported texture state: {} {}",
                static_cast<uint32_t>(_state),
                static_cast<uint32_t>(_pass_type),
                _is_write
            );
    }
...
void D3D12ResourceStateTracker::RecordState(D3D12Texture* texture, TextureStateDescription _required_state) {
    if (auto it = texture_states.find(texture); it != texture_states.end()) {
        ...
        if (_required_state.layout != D3D12_BARRIER_LAYOUT_UNDEFINED &&
        ...
        texture_states[texture] = TextureState{.before = init, .after = _required_state, .initial = init};
    }
...
void D3D12ResourceStateTracker::RecordState(
    ...
    switch (_state) {
        ...
        case EBufferState::INDIRECT:
            ...
            DASSERT(_pass_type != EPassType::Copy);
            desc = {
                .sync = D3D12_BARRIER_SYNC_EXECUTE_INDIRECT, .access = D3D12_BARRIER_ACCESS_INDIRECT_ARGUMENT
            };
            break;
...
void D3D12ResourceStateTracker::ResolveBarriers() {
    ...
    for (auto& buf : pending_buffers) {
        ...
        buffer_barriers.emplace_back(CD3DX12_BUFFER_BARRIER(
            state.before.sync, state.after.sync, state.before.access, state.after.access, buf->Native()
        ));
...
void D3D12ResourceStateTracker::RestoreState() {
    ...
    // maybe not need
    const auto buffer_init_state =
        BufferStateDescription{.sync = D3D12_BARRIER_SYNC_NONE, .access = D3D12_BARRIER_ACCESS_NO_ACCESS};
...
D3D12DescriptorHeapBase::D3D12DescriptorHeapBase(
    ...
    heapDesc.NodeMask       = 0;
    heapDesc.Flags =
        _is_shader_visible ? D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE : D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
    DX_CHECK_HRESULT(
        device->Native()->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(heap.ReleaseAndGetAddressOf()))
    );
...
D3D12GpuDescriptorAllocator::D3D12GpuDescriptorAllocator(
    D3D12Device*               _device,
    D3D12_DESCRIPTOR_HEAP_TYPE _type,
    ...
    D3D12GpuDescriptorHeap(
        _device,
        _type,
        std::min(_num_descriptors_total, _device->GetFeatureSupport().MaxViewDescriptorHeapSize() / 2)
    ),
