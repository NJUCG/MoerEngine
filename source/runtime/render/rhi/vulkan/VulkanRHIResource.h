...
static uint64 EncodeBindlessInfo(
    int   _texture_set,
    int   _buffer_set,
    uint  _stage_flags,
    uint8 _has_texture,
    ...
    return uint64(_texture_set << 17) | uint64(_buffer_set << 2) | uint64(_has_texture << 1) |
           uint64(_has_buffer) | (uint64(_stage_flags) << 32);
}
...
public:
    ...
    METoVKDescriptorType(EShaderParameterType _type, EShaderCodeResourceBindingType _binding_type);
...
public:
    VulkanPipelineState(VulkanDevice* _device, EType _type = EType::GFX) :
        PipelineState(),
...
public:
    ...
    struct Handle {
        ...
        Handle(uint64 _ptr, uint _slot, uint _attrib, uint _type) :
            ptr_1(_ptr >> 32),
            ptr_2(_ptr & 0xffffffff),
            slot(_slot),
            attrib(_attrib),
            type(_type) {}
        ...
        uint64 Ptr() const {
            return uint64(ptr_1) << 32 | uint64(ptr_2);
        }
    };
    ...
    void OnFree(
        ...
        const Array<uint>& _textures_freed,
        const Array<uint>& _buffers_freed
    );

    uint64 ArrayHandle() const override {
        return (uint64)bindless_array_buffer;
    }
...
class VulkanRaytracingGeometry final : public RaytracingGeometry, public VulkanDeviceObject {
public:
...
#define RESOURCE_CAST(RHIType, VkType)           \
    inline VkType* ResourceCast(RHIType* _val) { \
        return static_cast<VkType*>(_val);       \
    }
...
public:
    ...
    VkSemaphore GetUnderlyingHandle() {
        return timeline;
    }
...
public:
    ...
    static VkBuildAccelerationStructureFlagsKHR
    METoVKBuildAccelerationStructureFlagsKHR(ERayTracingAccelerationStructureBuildFlags _me_flags);
