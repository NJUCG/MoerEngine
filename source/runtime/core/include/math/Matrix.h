...
struct Matrix<T, 2, 2> {
    ...
    explicit Matrix(const Vector<T, 2>& r0, const Vector<T, 2>& r1) noexcept : r0(r0), r1(r1) {}
    explicit Matrix(T m00, T m01, T m10, T m11) noexcept : r0(m00, m01), r1(m10, m11) {}
    explicit Matrix(T (&e)[4]) noexcept {
        std::copy(e, e + 4, this->e);
    }
    explicit Matrix(const T* e) noexcept {
        std::copy(e, e + 4, this->e);
    }
    ...
    Vector<T, 2> operator[](size_t i) const {
        return re[i];
    }
    Vector<T, 2>& operator[](size_t i) {
        return re[i];
    }

    Vector<T, 2> GetColumn(size_t i) const noexcept {
        return Vector<T, 2>(re[0][i], re[1][i]);
    }
...
struct Matrix<T, 3, 3> {
    ...
    explicit Matrix(T m00, T m01, T m02, T m10, T m11, T m12, T m20, T m21, T m22) noexcept :
        ...
        r1(m10, m11, m12),
        r2(m20, m21, m22) {}
    explicit Matrix(T (&e)[9]) noexcept {
        std::copy(e, e + 9, this->e);
    }
    explicit Matrix(const T* e) noexcept {
        std::copy(e, e + 9, this->e);
    }
    ...
    Vector<T, 3> operator[](size_t i) const {
        return re[i];
    }
    Vector<T, 3>& operator[](size_t i) {
        return re[i];
    }
...
struct Matrix<T, 3, 4> {
    ...
    explicit Matrix(
        ...
        r1(m10, m11, m12, m13),
        r2(m20, m21, m22, m23) {}
    explicit Matrix(T (&e)[12]) noexcept {
        std::copy(e, e + 12, this->e);
    }
    explicit Matrix(const T* e) noexcept {
        std::copy(e, e + 12, this->e);
    }
    ...
    Vector<T, 4> operator[](size_t i) const {
        return re[i];
    }
    Vector<T, 4>& operator[](size_t i) {
        return re[i];
    }
...
struct Matrix<T, 4, 4> {
    ...
    explicit Matrix(
        ...
        r2(m20, m21, m22, m23),
        r3(m30, m31, m32, m33) {}
    explicit Matrix(T (&e)[16]) noexcept {
        std::copy(e, e + 16, this->e);
    }
    explicit Matrix(const T* e) noexcept {
        std::copy(e, e + 16, this->e);
    }
    ...
    Vector<T, 4> GetColumn(size_t i) const noexcept {
        return Vector<T, 4>(re[0][i], re[1][i], re[2][i], re[3][i]);
    }
