...
public:
    ...
    const Moer::UnorderedMap<std::string, std::string> Get(std::string section) const;
    ...
    template<typename T = std::string>
    Moer::Array<T> GetVector(const std::string& section, const std::string& name) const;

    template<typename T>
    Moer::Array<T>
    GetVector(const std::string& section, const std::string& name, const Moer::Array<T>& default_v) const;
...
protected:
    int                                                                           _error;
    Moer::UnorderedMap<std::string, Moer::UnorderedMap<std::string, std::string>> _values;
...
inline const Moer::UnorderedMap<std::string, std::string> INIReader::Get(std::string section) const {
...
template<typename T>
inline T INIReader::Get(const std::string& section, const std::string& name) const {
    auto const _section = Get(section);
...
template<typename T>
inline T INIReader::Get(const std::string& section, const std::string& name, T&& default_v) const {
    try {
...
template<typename T>
inline Moer::Array<T> INIReader::GetVector(const std::string& section, const std::string& name) const {
    std::string value = Get(section, name);

    std::istringstream             out{value};
    const Moer::Array<std::string> strs{
        std::istream_iterator<std::string>{out}, std::istream_iterator<std::string>()
    };
    try {
        Moer::Array<T> vs{};
...
template<typename T>
inline Moer::Array<T>
INIReader::GetVector(const std::string& section, const std::string& name, const Moer::Array<T>& default_v)
    const {
    try {
...
template<typename T>
inline void INIReader::InsertEntry(const std::string& section, const std::string& name, const T& v) {
...
template<typename T>
inline void
INIReader::InsertEntry(const std::string& section, const std::string& name, const Moer::Array<T>& vs) {
    if (_values[section][name].size() > 0) {
        throw std::runtime_error("duplicate key '" + std::string(name) + "' in section '" + section + "'.");
    }
...
template<typename T>
inline void INIReader::UpdateEntry(const std::string& section, const std::string& name, const T& v) {
...
template<typename T>
inline void
INIReader::UpdateEntry(const std::string& section, const std::string& name, const Moer::Array<T>& vs) {
    if (!_values[section][name].size()) {
        throw std::runtime_error("key '" + std::string(name) + "' not exist in section '" + section + "'.");
    }
    _values[section][name] = Vec2String(vs);
