...
class MTask {
public:
    MTask(EThread::Type _thread_to_return, std::string _info) : m_thread_to_return{_thread_to_return}, m_info{_info} {}
...
bool Moer::TaskGraphTest() {
    ...
    {// a task is not executed until its prerequisite is completed
        for (int i = 0; i != 100000; ++i) {
            ...
            GraphEventRef main_task = LambdaTask::Create([&b_executed] { b_executed = true; }).Wait(prereq).Dispatch();
    ...
    {// a task is not executed until all its prerequisites are completed
        for (int i = 0; i != 100000; ++i) {
            ...
            GraphEventRef   main_task = LambdaTask::Create([&b_executed] { b_executed = true; }).Wait(prereqs).Dispatch();
    ...
    for (int i = 0; i != 100000; ++i) {// a particular real-life case that doesn't work in the old TaskGraph if run in single-threaded mode.
        ...
        GraphEventRef local_queue_task = LambdaTask::Create([] {}).Wait(any_task).Dispatch(EThread::EMainThread);
        local_queue_task->Wait(EThread::EMainThread);
        assert(local_queue_task.GetRefCount() == 1);
    }
    ...
    {// launch a GT task, then an any-thread task that depends on it. wait for the any-thread task. this was a deadlock on the new frontend
        GraphEventRef gt_task = LambdaTask::Dispatch([] { return Moer::GetGameThreadId() == Platform::GetCurrentThreadID(); }, EThread::EMainThread);
