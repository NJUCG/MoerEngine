...
inline Moer::Matrix<T, 2, 2> Inverse(const Moer::Matrix<T, 2, 2>& m) noexcept {
    Moer::Matrix<T, 2, 2> ret;
...
template<Moer::NumericType T>
inline Moer::Matrix<T, 4, 4> Inverse(const Moer::Matrix<T, 4, 4>& m) noexcept {
    Moer::Matrix<T, 4, 4> ret;
...
template<Moer::NumericType T>
void QDUDecomposition(
    const Moer::Matrix<T, 3, 3>& m,
    Moer::Matrix<T, 3, 3>&       Q,
    Moer::Vector<T, 3>&          D,
    Moer::Vector<T, 3>&          U
) noexcept {
    ...
    T det = Q[0][0] * Q[1][1] * Q[2][2] + Q[0][1] * Q[1][2] * Q[2][0] + Q[0][2] * Q[1][0] * Q[2][1] -
...
/**
    ...
    */
Matrix4x4f
MakeLookatViewMatrixRH(const Vector3f& origin, const Vector3f& look_at, const Vector3f& up_dir) noexcept {
...
Matrix4x4d
MakeLookatViewMatrixRH(const Vector3d& origin, const Vector3d& look_at, const Vector3d& up_dir) noexcept {
...
/**
    ...
    */
Matrix4x4f
MakeLookatToWorldMatrixRH(const Vector3f& origin, const Vector3f& look_at, const Vector3f& up_dir) noexcept {
    auto y = Normalizef(up_dir);
    auto z = Normalizef(origin - look_at);
    auto x = Normalizef(Cross(y, z));
    y      = Cross(z, x);
...
Matrix4x4d
MakeLookatToWorldMatrixRH(const Vector3d& origin, const Vector3d& look_at, const Vector3d& up_dir) noexcept {
    auto y = Normalize(up_dir);
    auto z = Normalize(origin - look_at);
    auto x = Normalize(Cross(y, z));
    y      = Cross(z, x);
...
std::pair<Matrix4x4f, Matrix4x4f> MakeLookatViewMatrixWithInverseRH(
...
std::pair<Matrix4x4d, Matrix4x4d> MakeLookatViewMatrixWithInverseRH(
...
Matrix4x4f
MakePerspectiveMatrixRH(float fov_y, float aspect_ratio, float near_clip, float far_clip) noexcept {
...
Matrix4x4d
MakePerspectiveMatrixRH(double fov_y, double aspect_ratio, double near_clip, double far_clip) noexcept {
