...
private:
    ...
    static const tag_t stat_mask = 3ul ; //(1L<<48L)-1;
    static const uint32_t stat_sheft = 2;
...
public:
    ...
    tag_t get_stat() const
    {
        return extract_tag(ptr) & stat_mask;
    }

    tag_t get_next_tag() const
    {
        tag_t next = ((((get_tag() >> stat_sheft) + 1) << stat_sheft) | get_stat()) & std::numeric_limits<tag_t>::max();
        return next;
    }
    //tag_t get_next_stat() const {
    //    tag_t next = ((((get_tag() >> stat_sheft) + 1) << stat_sheft) | 1 ) & std::numeric_limits<tag_t>::max();
    //    return next;
    //}
