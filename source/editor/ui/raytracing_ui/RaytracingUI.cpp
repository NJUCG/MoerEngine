...
static constexpr std::string_view s_denoiser_mode_names[] = {"None", "ReBlur",
                                                             "Relax"};

static constexpr std::string_view s_aa_mode_names[] = {"TAA"};
static constexpr std::string_view s_jitter_mode_names[] = {"MSAA", "Halton",
                                                           "R2", "White Noise"};
static constexpr std::string_view s_final_color_names[] = {"SceneColor"
                                                           "DI",
                                                           "Emissive",
                                                           "Diffuse",
                                                           "Specular",
...
void RaytracingUI::ShowConfig() {
  ...
  if (ImGui::TreeNode("Process Light Configs")) {
    ...
    ImGui::SliderInt("Num Threads", &config.process_light_cfg.num_threads, 1,
                     32);
    ImGui::TreePop();
  }
  ...
  if (ImGui::TreeNode("Grid Config")) {
    ...
    ImGui::SliderInt("Light Per Ceil", &config.grid_config.light_per_ceil, 1,
                     1024);
    ...
    if (ImGui::TreeNode("InitialSampleSettings")) {
      if (ImGui::TreeNode("LocalLightSelection")) {
        for (auto &name : s_local_light_sample_mode_names) {
          ...
          if (ImGui::Selectable(name.data(),
                                config.restir_di_cfg.initial_sample_config
                                        .local_light_sample_mode == idx)) {
            config.restir_di_cfg.initial_sample_config.local_light_sample_mode =
                idx;
          }
        }
        ImGui::TreePop();
    ...
    if (ImGui::TreeNode("TemporalResampleSettings")) {
      if (ImGui::TreeNode("BiasCorrection")) {
        for (auto &name : s_bias_correction_mode_names) {
          ...
          if (ImGui::Selectable(name.data(),
                                config.restir_di_cfg.temporal_resample_config
                                        .bias_correction == idx)) {
      ...
      ImGui::SliderFloat(
          "depth threshold",
          &config.restir_di_cfg.temporal_resample_config.depth_threshold, 0.1f,
          30.0f);
      ImGui::SliderFloat(
          "normal threshold",
          &config.restir_di_cfg.temporal_resample_config.normal_threshold, 0.0f,
          1.0f);
      ImGui::TreePop();
    }
    ...
    if (ImGui::TreeNode("SpatialResampleSettings")) {
      if (ImGui::TreeNode("BiasCorrection")) {
        for (auto &name : s_bias_correction_mode_names) {
          uint idx = &name - s_bias_correction_mode_names;
          if (ImGui::Selectable(name.data(),
                                config.restir_di_cfg.spatial_resample_config
                                        .bias_correction == idx)) {
      ...
      ImGui::SliderFloat(
          "depth threshold",
          &config.restir_di_cfg.spatial_resample_config.depth_threshold, 0.0f,
          1.0f);
      ImGui::SliderFloat(
          "normal threshold",
          &config.restir_di_cfg.spatial_resample_config.normal_threshold, 0.0f,
          1.0f);
      ImGui::SliderInt(
          "num spatial samples",
          &config.restir_di_cfg.spatial_resample_config.num_spatial_samples, 1,
          32);
      ImGui::TreePop();
    }
  ...
  if (ImGui::TreeNode("ToneMapping")) {
    ImGui::SliderFloat("Histogram Low Percentile",
                       &config.tone_mapping_cfg.histogram_low_percentile, 0.0f,
                       1.0f);
    ImGui::SliderFloat("Histogram High Percentile",
                       &config.tone_mapping_cfg.histogram_high_percentile, 0.0f,
                       1.0f);
    ImGui::SliderFloat("Eye Adaptation Speed Up",
                       &config.tone_mapping_cfg.eye_adaptation_speed_up, 0.0f,
                       10.0f);
    ImGui::SliderFloat("Eye Adaptation Speed Down",
                       &config.tone_mapping_cfg.eye_adaptation_speed_down, 0.0f,
                       10.0f);
    ImGui::SliderFloat("Min Adapted Luminance",
                       &config.tone_mapping_cfg.min_adapted_luminance, 0.0f,
                       10.0f);
    ImGui::SliderFloat("Max Adapted Luminance",
                       &config.tone_mapping_cfg.max_adapted_luminance, 0.0f,
                       10.0f);
    ImGui::SliderFloat("Exposure Bias", &config.tone_mapping_cfg.exposure_bias,
                       -10.0f, 10.0f);
    ImGui::SliderFloat("WhitePoint", &config.tone_mapping_cfg.white_point, 0.0f,
                       10.0f);
    ImGui::TreePop();
  }
  if (ImGui::TreeNode("AntiAlias")) {
    if (ImGui::TreeNode("AA Settings")) {
      for (uint i = 0; i < EAnitiAliasMode::EAA_Num; i++) {
        if (ImGui::Selectable(s_aa_mode_names[i].data(),
                              config.aa_cfg.aa_mode == i)) {
    ...
    if (ImGui::TreeNode("Jitter Settings")) {
      for (uint i = 0; i < uint(AntialiasPass::EJitter::WhiteNoise) + 1; i++) {
        if (ImGui::Selectable(s_jitter_mode_names[i].data(),
                              uint(config.aa_cfg.jitter_mode) == i)) {
          config.aa_cfg.jitter_mode = (AntialiasPass::EJitter)i;
        }
      }
      ImGui::TreePop();
    }
    ImGui::SliderFloat("New Frame Weight", &config.aa_cfg.new_frame_weight,
                       0.0f, 1.0f);
    ImGui::SliderFloat("Clamping Factor", &config.aa_cfg.clamping_factor, 0.0f,
                       10.0f);
    ImGui::SliderFloat("Max Radiance", &config.aa_cfg.max_radiance, 0.0f,
                       10000.0f);
    ImGui::Checkbox("Enable History Clamping",
                    &config.aa_cfg.enable_history_clamping);
    ImGui::TreePop();
  }
  ImGui::Separator();
  ...
  for (uint i = 0; i < EOutputTexture::EOT_Num; i++) {
    if (ImGui::Selectable(s_exported_texture_names[i].data(),
                          config.export_cfg.output_texture == i)) {
