...
public:
  ...
  const UnorderedMap<std::string, TextureRef> &GetTextures() const {
    return textures;
  }
...
public:
  RTContext(ShaderUtils &_sd_utils, ImportanceSamplingContext &_is_ctx,
            BindlessArrayRef _bindless_array);

  void SetBindlessHandles(uint _geom_data_buf_handle,
  ...
  void CreateBuffersIfNeeded(uint _num_emissive_meshes,
                             uint _num_emissive_triangles,
                             uint _num_prim_lights, uint _num_geom_instance);
...
private:
  void AllocateAndFreeBdlsIfNeeded(uint &_target, const TextureView &_view,
                                   Sampler _sampler);
