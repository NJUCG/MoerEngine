...
PrepareLightPass::PrepareLightPass(RenderDevice &_device,
                                   ShaderManager &_manager, Scene &_scene)
    : device(_device), manager(_manager), scene(_scene),
      prepare_light_pipeline(
          ShaderManager::Get().Compute<PrepareLightShaderPipeline>(
              "lighting/PrepareLights.hlsl")) {}

void PrepareLightPass::CountEmissiveInstances(uint &_num_emissive_meshes,
...
static inline uint FloaT3ToR8G8B8Unorm(float _unpacked_input_x,
                                       float _unpacked_input_y,
                                       float _unpacked_input_z) {
...
static void PackPolyLightColor(float3 _color, PolymorphicLightInfo &_info) {
  ...
  uint packed_radiance =
      std::min(uint(ceilf(log_radiance * 65534.f)) + 1, 0xffffu);
  uint unpacked_radiance =
      std::exp2f((float(packed_radiance - 1) / 65534.f) *
                     (g_poly_morphic_light_max_log2_radiance -
                      g_poly_morphic_light_min_log2_radiance) +
                 g_poly_morphic_light_min_log2_radiance);

  _info.color_type_flags |= FloaT3ToR8G8B8Unorm(_color.x / unpacked_radiance,
                                                _color.y / unpacked_radiance,
                                                _color.z / unpacked_radiance);
...
static bool ConvertLight(LightComponent &_light, PolymorphicLightInfo &_info) {
  ...
  case Moer::ELightComponentType::DIRECTIONAL: {
    ...
    _info.color_type_flags = (uint)EPolyLightType::ELDirectional
                             << g_poly_morphic_light_type_shift;
    PackPolyLightColor(radiance, _info);
    _info.direction1 =
        PackNormalizedVector(Normalizef(dir_light->GetDirection()));
    _info.scalars =
        Fp32ToFp16(half_angluar_size_rad) | (Fp32ToFp16(solid_angle) << 16);
    return true;
    break;
  }
  case Moer::ELightComponentType::SPOT: {
    ...
    float softness = Saturate(1.f / (spot_light->GetOuterConeAngle() -
                                     spot_light->GetInnerConeAngle()));
    ...
    _info.primary_axis =
        PackNormalizedVector(Normalizef(spot_light->GetDirection()));
  ...
  case Moer::ELightComponentType::POINT: {
    PointLightComponent *point_light =
        static_cast<PointLightComponent *>(&_light);
  ...
  case Moer::ELightComponentType::ENVIRONMENT: {
    ...
    if (!env_light->bdls_handle)
      return false;
    _info.color_type_flags = (uint)EPolyLightType::ELEnv
                             << g_poly_morphic_light_type_shift;
...
void PrepareLightPass::Process(CommandList &_cmd_list, RTContext &_rt_ctx) {
  ...
  Array<uint> geo_instance_to_light(scene.GetGeometryInstances().size(),
                                    s_invalid_light_idx);
  ...
  scene.ForEach([&](Entity _entity) {
    {
      const MeshInfo &mesh_info =
          *RenderableManager::Get().GetMeshInfo(_entity).get();
      ...
      for (uint i = 0; i < mesh_info.geometries.size(); ++i) {
        ...
        geo_instance_to_light[scene.GetInstanceDatas()[instance_id]
                                  .first_geom_instance_idx +
                              i] = light_buf_offset;
        ...
        task.prev_light_offset =
            iter == instance_light_buffer_offsets.end() ? -1 : iter->second;
  ...
  if (_rt_ctx.b_parallel_process_light) {
    ...
    Array<UnorderedMap<uint64, uint>> prev_light_buffer_offsets_parallel(
        parrallel_cnt);
    for (uint i = 0; i < parrallel_cnt; i++) {
      // reserve space for normal lights
      entity_idx_parrallel[i][0].reserve(light_entities_src.size() /
                                         parrallel_cnt);
    }
    ...
    ParallelFor(parrallel_cnt, [&](uint _idx) {
      ...
      uint region_end =
          std::min((_idx + 1) * light_entities_src.size() / parrallel_cnt,
                   light_entities_src.size());
    ...
    ParallelFor(parrallel_cnt, [&](uint _idx) {
      for (uint light_type = 0; light_type < 2; light_type++) {
        if (entity_idx_parrallel[_idx][light_type].size() > 0) {
          uint cur_offset = light_cnt[_idx][light_type] -
                            entity_idx_parrallel[_idx][light_type].size();
          uint cur_task_offset = light_cnt[_idx][light_type] -
                                 entity_idx_parrallel[_idx][light_type].size() +
                                 task_offset;
          uint total_light_offset = cur_offset + light_buf_offset;
          for (uint i = 0; i < entity_idx_parrallel[_idx][light_type].size();
               i++) {
            PrepareLightsTask task{};
            ...
            task.prev_light_offset =
                iter == primitive_light_buffer_offsets.end() ? -1
                                                             : iter->second;
            tasks[cur_task_offset + i] = task;
            prev_light_buffer_offsets_parallel[_idx][key] =
                total_light_offset + i;
          }
          ...
          for (uint i = 0; i < entity_idx_parrallel[_idx][light_type].size();
            ...
            if (!ConvertLight(
                    *LightComponentManager::Get().Get(
                        light_entities_src
                            [entity_idx_parrallel[_idx][light_type][i]]),
                    light_info)) {
    ...
    for (uint i = 0; i < parrallel_cnt; i++) {
      primitive_light_buffer_offsets.insert(
          prev_light_buffer_offsets_parallel[i].begin(),
          prev_light_buffer_offsets_parallel[i].end());
    }
    ...
    if (auto cur_env = this->scene.GetCurrentEnvMap(); cur_env.texture) {
      ...
      PolymorphicLightInfo light_info{};
      auto pre_iter =
          primitive_light_buffer_offsets.find(uint64(env_comp.Get()));
      if (ConvertLight(*env_comp, light_info)) {
        ...
        task.num_triangles = 1;
        task.prev_light_offset =
            pre_iter == primitive_light_buffer_offsets.end() ? -1
                                                             : pre_iter->second;
        primitive_light_buffer_offsets[uint64(env_comp.Get())] =
            light_buf_offset;
        light_buf_offset += task.num_triangles;
        tasks.emplace_back(task);
    ...
    Array<Entity> light_entities(light_entities_src.begin(),
                                 light_entities_src.end());

    std::sort(light_entities.begin(), light_entities.end(),
              [](Entity _lhs, Entity _rhs) {
    ...
    for (auto &entity : light_entities) {
      ...
      auto pre_iter =
          primitive_light_buffer_offsets.find(uint64(light_data.Get()));

      PrepareLightsTask task{};
      task.instance_geo_idx = prim_light_infos.size() | g_task_prim_light_bit;
      task.light_offset = light_buf_offset;
      task.num_triangles = 1;
      task.prev_light_offset = pre_iter == primitive_light_buffer_offsets.end()
                                   ? -1
                                   : pre_iter->second;

      primitive_light_buffer_offsets[uint64(light_data.Get())] =
          light_buf_offset;
      light_buf_offset += task.num_triangles;
      tasks.emplace_back(task);
      prim_light_infos.emplace_back(light_info);
  ...
  _cmd_list.CopyFrom(
      std::span<byte>((byte *)geo_instance_to_light.data(),
                      geo_instance_to_light.size() * sizeof(uint)),
      _rt_ctx.geo_instance_to_light_buf->GetView());
  _cmd_list.CopyFrom(
      std::span<byte>((byte *)tasks.data(),
  ...
  if (!prim_light_infos.empty()) {
    _cmd_list.CopyFrom(
        std::span<byte>((byte *)prim_light_infos.data(),
                        prim_light_infos.size() * sizeof(PolymorphicLightInfo)),
        _rt_ctx.prim_light_buf->GetView());
  }

  _cmd_list.ClearResource(_rt_ctx.light_mapping_buf->GetView(), 0u);
  _cmd_list.ClearResource(_rt_ctx.local_light_pdf_tex->GetView(
                              0, _rt_ctx.local_light_pdf_tex->GetNumMips()),
                          float4(0.f));
  ...
  _rt_ctx.is_ctx.SetLightBufferParams(
      param.cur_light_offset, num_finite_prim_lights + num_prim_lights,
      num_infinite_prim_lights, num_is_env_lights);

  _cmd_list
      .Compute(prepare_light_pipeline, param, _rt_ctx.light_data_buf->GetView(),
               _rt_ctx.light_mapping_buf->GetView(),
               _rt_ctx.local_light_pdf_tex->GetView(),
               _rt_ctx.prim_light_buf->GetView(), _rt_ctx.task_buf->GetView(),
  ...
  // num_is_env_lights;
  _cmd_list.AddCallback(
      [geo_instance_to_light(std::move(geo_instance_to_light)),
