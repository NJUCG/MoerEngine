...
public:
  ...
  DEFINE_SHADER_ARGS(param, rt_config, out_normal_roughness,
                     out_basecolor_metalness, out_direct_lighting, out_emission,
                     out_diffuse, out_specular, out_view_z, out_mv,
                     out_shadow_info, out_scene_color, bdls, tlas);
};
...
void RaytracingMain(SharedPtr<EditorUI> editor_ui) {
  ...
  auto resolution =
      editor_ui
          ->GetResolution(); // TODO: 是否要从WindowContext中获取resolution?
  ...
  GraphEventRef load_res_event =
      LambdaTask::Create([&rt_res, &b_new_env_map]() {
  ...
  BufferRef rt_config_param_buffer = device.CreateBuffer<Moer::byte>(
      sizeof(RTConfigParam) * 1, EBufferUsageFlags::CONSTANT_BUFFER);
  ...
  SampleTexturePipeline sample_tex =
      manager.Raster()
  ...
  TextureRef output = device.CreateTexture(
      Extent2D(resolution.x, resolution.y), PF_R8G8B8A8_SRGB,
      ETextureUsageFlags::COLOR_ATTACHMENT);

  TextureRef ui_frame_buffer = device.CreateTexture(
      "ui_frame_buffer", Extent2D(resolution.x, resolution.y), PF_R8G8B8A8_SRGB,
  ...
  auto create_frame_buffers = [&](uint2 _new_extent) {
    output = device.CreateTexture(
        "output", Extent2D(_new_extent.x, _new_extent.y), PF_R8G8B8A8_SRGB,
        ETextureUsageFlags::COLOR_ATTACHMENT);

    ui_frame_buffer = device.CreateTexture(
        "ui_frame_buffer", Extent2D(_new_extent.x, _new_extent.y),
        PF_R8G8B8A8_SRGB,
        ETextureUsageFlags::COLOR_ATTACHMENT | ETextureUsageFlags::SAMPLED);
  };
  ...
  std::filesystem::path exported_file_path =
      ConfigManager::GetInstance().GetWorkspacePath() / "saved";
  ...
  UniquePtr<PrepareLightPass> prepare_light_pass =
      MakeUnique<PrepareLightPass>(device, manager, scene);
  UniquePtr<GBufferPass> g_buffer_pass =
      MakeUnique<GBufferPass>(device, manager, scene);
  UniquePtr<LightingPass> lighting_pass =
      MakeUnique<LightingPass>(manager, scene);
  UniquePtr<CompositionPass> composition_pass =
      MakeUnique<CompositionPass>(device, manager, scene);
  UniquePtr<VisualizePass> visualize_pass =
      MakeUnique<VisualizePass>(device, manager);
  UniquePtr<RTContext> rt_ctx =
      MakeUnique<RTContext>(sd_utils, is_ctx, bindless_array);
  UniquePtr<ToneMappingPass> tone_mapping_pass;
  UniquePtr<UiCombinePass> ui_combine_pass =
      MakeUnique<UiCombinePass>(manager, output->GetFormat());
  ...
  auto nrd_interface =
      nrd_ext->CreateInterface(max_frame_in_flight, resolution.x, resolution.y);
  ...
  while (WindowContext::ShouldClose(WindowContext::GetMainWindow()) == false) {
    ...
    RaytracingConfig &ui_config =
        editor_ui->m_raytracing_ui.GetEditableConfig();

    timer.Stop();
    auto frame_time = timer.ElapsedMilliseconds();
    timer.Start();

    WindowContext::GetWindowSize(WindowContext::GetMainWindow(), &w_width,
                                 &w_height);
    ...
    if (w_width != resolution.x || w_height != resolution.y) {
      ...
      output = device.CreateTexture(
          "output", Extent2D(resolution.x, resolution.y), PF_R8G8B8A8_SRGB,
          ETextureUsageFlags::COLOR_ATTACHMENT);
      ...
      nrd_interface = nrd_ext->RecreateInterface(std::move(nrd_interface),
                                                 resolution.x, resolution.y);

      antialias_pass_info.motion = rt_ctx->frame_rt.motion;
      antialias_pass_info.feedback_color_ping =
          rt_ctx->frame_rt.feedback_color_ping;
      antialias_pass_info.feedback_color_pong =
          rt_ctx->frame_rt.feedback_color_pong;
      antialias_pass_info.resolved_color = rt_ctx->frame_rt.resolved_color;
      antialias_pass_info.hdr_color = rt_ctx->frame_rt.hdr_color;
      antialias_pass = MakeUnique<AntialiasPass>(device, manager, scene,
                                                 antialias_pass_info);
      b_feedback_valid = false;
    }

    if (Scene::GetCurrentSceneLoadInfo().Get() &&
        Scene::GetCurrentSceneLoadInfo()->IsReady() &&
      ...
      if (first_load) {
        ...
        rt_ctx->SetBindlessHandles(geometry_buffer_handle,
                                   instance_buffer_handle, material_buffer_idx);
        ...
        scene.ForEach([&](Entity _entity) {
          ...
          RaytracingGeometryRef blas =
              device.CreateRaytracingGeometry(rt_geo_info);
          rt_geometries.push_back(blas);

          auto &instance = rt_scene->AddInstance();
          instance.geom = blas;
          instance.transform =
              TransformManager::Get().Get(_entity).GetMatrix3x4();
...

        {
          ...
          if (first_load) {
            ...
            sampled_textures.reserve(
                (scene.GetGpuScene().material_textures.size()));

            for (auto &[name, tex] : scene.GetGpuScene().material_textures) {
              sampled_textures.emplace_back(ImportTexture(
                  tex.texture->GetView(0, tex.texture->GetNumMips()),
                  ETextureState::SAMPLE));
            }

            cmd_list.ImportTextureFromQueue(EQueueType::Copy,
                                            std::move(sampled_textures));
          }
        ...
        editor_ui->RegisterUIFunc(
            ...
            [&scene, &selected_material_texture_name, &b_use_bindless,
             &b_final_show_texture, &mip_level]() {
              ...
              if (ImGui::TreeNode("MaterialTexture")) {
                for (auto &[name, tex] :
                     scene.GetGpuScene().material_textures) {
                  // selectable
        ...
        // cmd_list.UpdateRaytracingScene(rt_scene);
        gfx_queue.Execute(cmd_list.Submit().Wait(copy_queue_timeline,
                                                 last_io_change_timeline));
        gfx_queue.Sync();
      }
...

        {
          ...
          if (first_load) {
            ...
            for (auto &[name, tex] : rt_res_textures) {
              import_textures.emplace_back(ImportTexture(
                  tex->GetView(0, tex->GetNumMips()), ETextureState::SAMPLE));
            }
            cmd_list.ImportTextureFromQueue(EQueueType::Copy,
                                            std::move(import_textures));
          }
          gfx_queue.Execute(cmd_list.Submit().Wait(
              copy_queue_timeline, copy_queue_timeline->GetValue()));
          gfx_queue.Sync();
        }
        env_map = rt_res.GetDefaultEnvMap();
        auto cur_env = scene.GetCurrentEnvMap();
        Moer::EnvironmentLightComponent *env_light =
            MoerNew(Moer::EnvironmentLightComponent)(float3(1.f),
                                                     env_map->GetExtent().xy);

        Sampler sampler{SF_CUBIC, SAM_REPEAT};
        env_light->bdls_handle = bindless_array->AllocateTexture(
            env_map->GetView(0, env_map->GetNumMips()), sampler);
      ...
      if (!tone_mapping_pass) {
        ...
        tone_mapping_pass =
            MakeUnique<ToneMappingPass>(device, manager, scene, info);
      }
      ...
      // prepare frame
      {
        ...
        rt_config_param.world2clip =
            Transpose(camera->GetViewProjectionMatrix());

        rt_config_param.tan_pixel_angular_radius =
            tanf(Angle::DegreeToRadian(camera->GetFov()));
        ...
        rt_config_param.sun_direction_gexposure =
            float4(sun_dir, ui_config.exposure);
        cmd_list.CopyFrom(std::span<Moer::byte>((Moer::byte *)&rt_config_param,
                                                sizeof(RTConfigParam)),
      ...
      // fill ui data
      {
        ...
        rt_ctx->config.reblur_diffuse_hit_dist_params =
            *reinterpret_cast<const float4 *>(
                &ui_cfg.denoiser_cfg.hit_dist_params);
        ...
        tone_params.eye_adaptation_speed_down =
            tone_cfg.eye_adaptation_speed_down;
        ...
        tone_params.histogram_low_percentile =
            tone_cfg.histogram_low_percentile;
        tone_params.histogram_high_percentile =
            tone_cfg.histogram_high_percentile;
        ...
        rt_ctx->b_parallel_process_light =
            ui_config.process_light_cfg.parallel_mode;
      ...
      prepare_light_pass->CountEmissiveInstances(num_emissive_meshes,
                                                 num_emissive_triangles);

      rt_ctx->CreateBuffersIfNeeded(num_emissive_meshes, num_emissive_triangles,
      ...
      //////////////////////////////////////////////////////////////////////////
      {
        ...
        if (denoiser != nrd::Denoiser::MAX_NUM) {
          ...
          nrd_interface->UpdateCommonSettings(
              nrd_time++, Vector2ui(resolution.x, resolution.y),
          ...
          // opaque
          {
            nrd_interface->SetInput(
                ...
                rt_ctx->frame_rt.motion);
            nrd_interface->SetInput(
                Ext::NRDInterface::EResourceSlot::NORMAL_ROUGHNESS,
                frame_rt.normal_roughness);
            nrd_interface->SetInput(Ext::NRDInterface::EResourceSlot::VIEW_Z,
                                    b_current_frame ? frame_rt.view_depth
                                                    : frame_rt.prev_view_depth);
            ...
            nrd_interface->SetInput(
                ...
                frame_rt.diffuse_lighting);
            nrd_interface->SetInput(
                Ext::NRDInterface::EResourceSlot::IN_SPECULAR,
                frame_rt.specular_lighting);
      ...
      antialias_pass->Process(cmd_list, *rt_ctx, aa_params, b_feedback_valid,
                              rt_ctx->frame_rt.hdr_color,
                              rt_ctx->frame_rt.resolved_color);
      tone_mapping_pass->Process(cmd_list, *rt_ctx, tone_params,
                                 rt_ctx->frame_rt.resolved_color,
                                 rt_ctx->frame_rt.ldr_color);
      visualize_pass->Process(cmd_list, *rt_ctx, visualize_config,
                              bindless_array);
      if (b_final_show_texture &&
          scene.GetGpuScene().material_textures.contains(
              selected_material_texture_name)) {
        TextureRef texture =
            scene.GetGpuScene()
                .material_textures[selected_material_texture_name]
                .texture;
        ...
        sd_utils.ShowTexture(cmd_list, bindless_array, show_texture_params,
                             texture, rt_ctx->frame_rt.ldr_color);
      }
      ...
      //////////////////////////////////////////////////////////////////////////
      {
        auto dequantentize_half = [](short _h, bool _gamma_correct = true) {
          ...
          if (_gamma_correct) {
            u.f = u.f <= 0.0031308f
                      ? 12.92f * u.f
                      : 1.055f * std::pow(u.f, 1.f / 2.4f) - 0.055f;
          }
          return u.f;
        };
        ...
        if (b_export) {
          ...
          case EOT_LDR: {
            ...
            cmd_list.CopyFrom(rt_ctx->frame_rt.ldr_color->GetView(),
                              copy_back_data);
          ...
          case EOT_HDR: {
            ...
            copy_back_data.resize(size);
            cmd_list.CopyFrom(rt_ctx->frame_rt.resolved_color->GetView(),
                              copy_back_data);
          ...
          if (size != 0) {
            ...
            if (hdr) {
              ...
              assert(rt_ctx->frame_rt.resolved_color->GetFormat() ==
              ...
              ParallelFor(range_cnt, [&](uint _idx) {
                ...
                for (size_t i = start; i < copy_back_data.size() && i < end;
                     i += 8) {
              ...
              assert(rt_ctx->frame_rt.ldr_color->GetFormat() ==
                         PF_R8G8B8A8_UNORM &&
                     "ldr format must be r8g8b8a8_unorm");
              ...
              ParallelFor(range_cnt, [&](uint _idx) {
                ...
                for (size_t i = start; i < copy_back_data.size() && i < end;
                     i += 4) {
                  copy_back_data[i] = (Moer::byte)dequantentize_byte_to_srgb(
                      ubyte(copy_back_data[i]));
                  copy_back_data[i + 1] =
                      (Moer::byte)dequantentize_byte_to_srgb(
                          ubyte(copy_back_data[i + 1]));
                  copy_back_data[i + 2] =
                      (Moer::byte)dequantentize_byte_to_srgb(
                          ubyte(copy_back_data[i + 2]));
                  copy_back_data[i + 3] =
                      (Moer::byte)dequantentize_byte_to_srgb(
                          ubyte(copy_back_data[i + 3]));
                }
              });
              stbi_write_png(
                  (exported_file_path / file_name).generic_string().data(),
                  resolution.x, resolution.y, 4, (void *)copy_back_data.data(),
                  4 * resolution.x);
    ...
    TextureRef final_color = b_final_show_texture
                                 ? rt_ctx->frame_rt.ldr_color
                                 : rt_ctx->frame_rt.debug_color;

    ui_combine_pass->Process(cmd_list, resolution, final_color, ui_frame_buffer,
                             output, editor_ui);
