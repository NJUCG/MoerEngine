...
namespace Moer{
    ...
    namespace SampleFunc{
        ...
        void SamplePdfMip(
            ...
            for(int mip = last_mip; mip >= 0; mip--){
                ...
                float4 pdf_samples = float4(
                    max(0.f, _pdf_tex.Load(int3(_pos.x, _pos.y, mip))),
                ...
                float pdf_sum = pdf_samples.x + pdf_samples.y + pdf_samples.z + pdf_samples.w;
                if(pdf_sum <= 0.f){
                    pdf = 0.f;
                    return;
                }
        ...
        void SampleLocalLights(
            inout RandomState _rng,
            Texture2D<float> _pdf_tex,
            uint2 _tex_size,
            uint _tile_idx,
            uint _sample_in_tile,
            ...
            uint light_idx = Math::LinearIndexToZCurve(tex_pos);
    ...
    void SampleEnvMap(
        ...
        uint packed_uv = uint(saturate(uv.x) * 0xffff) | (uint(saturate(uv.y) * 0xffff) << 16);
    ...
    void SampleLocalLightsForGrid(
        ...
        uint light_in_ceil = _light_slot % _grid_params.common_params.lights_per_ceil;
        uint ceil_idx = _light_slot / _grid_params.common_params.lights_per_ceil;

        float3 ceil_center;
        float ceil_radius;

        if(!Grid::WorldPosFromCellIdx(_grid_params, int(ceil_idx), ceil_center, ceil_radius)){
            rw_ris_buffer[ris_idx] = uint2(0, 0);
            return;
        }
        ...
        PolymorphicLightInfo light_info = PolymorphicLightInfo::EmptyLightInfo();
        uint seleted = 0;
        float selected_target_pdf = 0.f;
        float weight_sum = 0.f;

        float inv_num_samples = 1.f / float(_grid_params.common_params.num_build_samples);

        DI::LocalLightSelectionContext ctx;
        switch(_grid_params.common_params.local_light_sampling_mode){
            case DI::s_di_local_light_sample_mode_power_ris:
        ...
        for(uint i = 0; i < _grid_params.common_params.num_build_samples; i++){
            ...
            PolymorphicLightInfo cur_light_info = PolymorphicLightInfo::EmptyLightInfo();
            ...
            float target_pdf = PolymorphicLight::GetVolumeWeight(cur_light_info, ceil_center, ceil_radius);
        ...
        float weight = (selected_target_pdf > 0.f) ? weight_sum / selected_target_pdf : 0.f;
        bool compact = false;
        ...
        if(compact){
            seleted |= DI::s_di_light_compact_bit;
        }
