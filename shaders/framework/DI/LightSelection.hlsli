...
namespace Moer{
    ...
    namespace DI{
        ...
        void RandomlySelectLightFromRISTile(
            ...
            GetLightInfoFromRisData(tile_data, ris_buf_idx, _light_info, _light_idx, _inv_pdf);
        }
        ...
        void RandomlySelectLightDataUniformly(
            inout RandomState _rng,
            LightBufferRegion _region,
            out PolymorphicLightInfo _light_data,
            out uint _light_idx,
            out float _inv_pdf
        ){
            float rng = _rng.GetFloat();
            _inv_pdf = 1.f / float(_region.light_cnt);
            _light_idx = min(uint(floor(rng * _region.light_cnt)), _region.light_cnt - 1) + _region.first_light_idx;
            _light_info = LoadLightInfo(_light_idx);
        }
        ...
        struct LocalLightSelectionContext{
            ...
            static LocalLightSelectionContext CreateUniform(LightBufferRegion _light_region){
            ...
            static LocalLightSelectionContext CreatePowerRIS(inout RandomState _rng, RISBufferSegmentParams _ris_params){
                LocalLightSelectionContext result;
                result.sample_mode = sd_di_local_light_sample_mode_power_ris;
            ...
            void SelectNext(
                ...
                switch(sample_mode){
                    ...
                    case sd_di_local_light_sample_mode_power_ris:
                        RandomlySelectLightFromRISTile(_rng, ris_tile_info, _light_info, _light_idx, _inv_pdf);
                        break;

                    default:
                    case sd_di_local_light_sample_mode_uniform:
                        RandomlySelectLightDataUniformly(_rng, light_region, _light_info, _light_idx, _inv_pdf);
                        break;
                }
            }
        };
