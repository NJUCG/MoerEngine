...
namespace Moer{
    namespace DI{
        struct SampleConfigs{
            ...
            static SampleConfigs Create(
                uint _num_local_light,
                ...
                result.local_light_mis_weight = float(_num_local_light) / float(result.num_mis);
                result.env_map_mis_weight = float(_num_infinite_light) / float(result.num_mis);
        ...
        Reservoir SampleEnvMap(
            ...
            if(!_env_light_params.light_valid)
                return res;
