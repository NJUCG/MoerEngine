...
float3 GetMotion(Moer::ViewParam _view, int2 _pixel_pos){
  ...
  TextureHandle prev_normal_handle = (TextureHandle)param.bindless_handles.gbuffer_prev_normal;
  ...
  float3 prev_normal = Math::OctToNdirUnorm32(prev_normal_handle.SampleLevel<uint>(cur_uv, 0));
  float3 normal = GetNormal(_view, _pixel_pos);
    ...
    if (!Math::IsValidNeighbor(
            normal, prev_normal,
            expected_depth_linear, prev_depth,
            0.4f, 2.f)) {
