...
[[vk::binding(0, 0)]]ConstantBuffer<ToneMappingParams> params : register(b0);
Texture2D source_tex : register(t0);
Buffer<uint> exposuer : register(t1);

Texture2D color_lut : register(t2);
SamplerState color_lut_sampler : register(s0);
...
float3 ConvertToLDR(float3 hdr_color) {
  ...
  float scaled_luminance = src_luminance * params.exposure_scale / adapted_luminance;
  float mapped_luminance = scaled_luminance * (1.0f + scaled_luminance / params.white_point_inv_squared) / (1.0f + scaled_luminance);

  return color * mapped_luminance / src_luminance;
}
...
float3 ApplyColorLUT(float3 color){
    ...
    float2 uv2 = float2((ceil(b) + 1.0f) * size + r, g) * params.color_lut_size_inv;
...
void main(in float4 pos : SV_Position,
    ...
    out float4 target : SV_Target){
        ...
        target.rgb = ConvertToLDR(hdr_color.rgb);
