...
// BRDF
#define STL_SPECULAR_DOMINANT_DIRECTION_DEFAULT                                \
  STL_SPECULAR_DOMINANT_DIRECTION_APPROX
...
void Swap(inout uint x, inout uint y) { _Swap(x, y); }

void Swap(inout uint2 x, inout uint2 y) { _Swap(x, y); }

void Swap(inout uint3 x, inout uint3 y) { _Swap(x, y); }

void Swap(inout uint4 x, inout uint4 y) { _Swap(x, y); }
...
float LinearStep(float a, float b, float x) { return _LinearStep(a, b, x); }

float2 LinearStep(float2 a, float2 b, float2 x) { return _LinearStep(a, b, x); }

float3 LinearStep(float3 a, float3 b, float3 x) { return _LinearStep(a, b, x); }

float4 LinearStep(float4 a, float4 b, float4 x) { return _LinearStep(a, b, x); }
...
float2 SmoothStep01(float2 x) { return _SmoothStep01(saturate(x)); }

float3 SmoothStep01(float3 x) { return _SmoothStep01(saturate(x)); }

float4 SmoothStep01(float4 x) { return _SmoothStep01(saturate(x)); }
...
// Pow
float Pow(float x, float y) { return pow(abs(x), y); }

float2 Pow(float2 x, float y) { return pow(abs(x), y); }

float2 Pow(float2 x, float2 y) { return pow(abs(x), y); }

float3 Pow(float3 x, float y) { return pow(abs(x), y); }

float3 Pow(float3 x, float3 y) { return pow(abs(x), y); }

float4 Pow(float4 x, float y) { return pow(abs(x), y); }

float4 Pow(float4 x, float4 y) { return pow(abs(x), y); }

// Pow for values in range [0; 1]
float Pow01(float x, float y) { return pow(saturate(x), y); }

float2 Pow01(float2 x, float y) { return pow(saturate(x), y); }

float2 Pow01(float2 x, float2 y) { return pow(saturate(x), y); }

float3 Pow01(float3 x, float y) { return pow(saturate(x), y); }

float3 Pow01(float3 x, float3 y) { return pow(saturate(x), y); }

float4 Pow01(float4 x, float y) { return pow(saturate(x), y); }

float4 Pow01(float4 x, float4 y) { return pow(saturate(x), y); }
...
float Sqrt01(float x) { return sqrt(saturate(x)); }

float2 Sqrt01(float2 x) { return sqrt(saturate(x)); }

float3 Sqrt01(float3 x) { return sqrt(saturate(x)); }

float4 Sqrt01(float4 x) { return sqrt(saturate(x)); }
...
float Rsqrt(float x, compiletime const uint mode = STL_RSQRT_DEFAULT) {
  if (mode <= STL_POSITIVE_RSQRT_BUILTIN_SAFE)
    return rsqrt(mode == STL_POSITIVE_RSQRT_BUILTIN ? x
                                                    : max(x, STL_SMALL_EPS));

  return 1.0 /
         sqrt(mode == STL_POSITIVE_RSQRT_ACCURATE ? x : max(x, STL_SMALL_EPS));
}
...
float AcosApprox(float x) { return _AcosApprox(x); }

float2 AcosApprox(float2 x) { return _AcosApprox(x); }

float3 AcosApprox(float3 x) { return _AcosApprox(x); }

float4 AcosApprox(float4 x) { return _AcosApprox(x); }
...
#define _AtanApprox(x)                                                         \
  (Math::Pi(0.25) * x - (abs(x) * x - x) * (0.2447 + 0.0663 * abs(x)))

float AtanApprox(float x) { return _AtanApprox(x); }

float2 AtanApprox(float2 x) { return _AtanApprox(x); }

float3 AtanApprox(float3 x) { return _AtanApprox(x); }

float4 AtanApprox(float4 x) { return _AtanApprox(x); }
...
float PositiveRcp(float x,
                  compiletime const uint mode = STL_POSITIVE_RCP_DEFAULT) {
...
// Manhattan distance
float ManhattanDistance(float2 a, float2 b) { return dot(abs(a - b), 1.0); }

float ManhattanDistance(float3 a, float3 b) { return dot(abs(a - b), 1.0); }
...
float3x3 GetRotator(float3 axis, float angle) {
  ...
  return float3x3(t1.x, t2.x, t3.x, t3.y, t1.y, t2.y, t2.z, t3.z, t1.z);
}

float4 GetRotator(float sa, float ca) { return float4(ca, sa, -sa, ca); }
...
float3 RotateVector(float4x4 m, float3 v) { return mul((float3x3)m, v); }

float3 RotateVector(float3x3 m, float3 v) { return mul(m, v); }
...
float3 AffineTransform(float3x4 m, float3 p) { return mul(m, float4(p, 1.0)); }

float3 AffineTransform(float4x4 m, float4 p) { return mul(m, p).xyz; }

float4 ProjectiveTransform(float4x4 m, float3 p) {
  return mul(m, float4(p, 1.0));
}

float4 ProjectiveTransform(float4x4 m, float4 p) { return mul(m, p); }

float2 GetPerpendicular(float2 v) { return float2(-v.y, v.x); }
...
float3x3 GetBasis(float3 N) {
  ...
  // Note: due to the quaternion formulation, the generated frame is rotated by
  // 180 degrees, s.t. if N = (0, 0, 1), then T = (-1, 0, 0) and B = (0, -1, 0).
...
// }
float3 ReconstructViewPosition(float2 uv, float4 cameraFrustum,
                               float viewZ = 1.0, float orthoMode = 0.0) {
  float3 p;
...
float2
GetPrevUvFromMotion(float2 uv, float3 X, float4x4 worldToClipPrev,
                    float3 motionVector,
                    compiletime const uint motionType = STL_WORLD_MOTION) {
  float3 Xprev = X + motionVector;
  float2 uvPrev = GetScreenUv(worldToClipPrev, Xprev);

  [flatten] if (motionType == STL_SCREEN_MOTION) uvPrev = uv + motionVector.xy;
...
float3x3 GetMirrorMatrix(float3 n) {
  float3x3 m;
  m[0] = float3(1.0 - 2.0 * n.x * n.x, 0.0 - 2.0 * n.y * n.x,
                0.0 - 2.0 * n.z * n.x);
  m[1] = float3(0.0 - 2.0 * n.x * n.y, 1.0 - 2.0 * n.y * n.y,
                0.0 - 2.0 * n.z * n.y);
  m[2] = float3(0.0 - 2.0 * n.x * n.z, 0.0 - 2.0 * n.y * n.z,
                1.0 - 2.0 * n.z * n.z);
...
float Luminance(float3 linearColor,
                compiletime const uint mode = STL_LUMINANCE_DEFAULT) {
  return dot(linearColor, mode == STL_LUMINANCE_BT601
                              ? float3(0.2990, 0.5870, 0.1140)
...
float3 LinearToSrgb(float3 color) {
  ...
  return lerp(consts.x * Math::Pow(color, consts.yyy) + consts.zzz,
              consts.w * color, color < 0.0031308);
}
...
float3 SrgbToLinear(float3 color) {
  ...
  return lerp(color * consts.x,
              Math::Pow(color * consts.y + consts.zzz, consts.www),
              color > 0.04045);
}
...
// Encode an RGB color into a 32-bit LogLuv HDR format. The supported luminance
// range is roughly 10^-6..10^6 in 0.17% steps. The log-luminance is encoded
// with 14 bits and chroma with 9 bits each. This was empirically more accurate
// than using 8 bit chroma. Black (all zeros) is handled exactly
uint LinearToLogLuv(float3 color) {
  ...
  //  v = 9Y / ( -2X + 12Y + 3(X + Y + Z) )
  float invDenom =
      1.0 / (-2.0 * XYZ.x + 12.0 * XYZ.y + 3.0 * (XYZ.x + XYZ.y + XYZ.z));
  float2 uv = float2(4.0, 9.0) * XYZ.xy * invDenom;

  // Encode chroma (u,v) in 9 bits each.
  // The gamut of perceivable uv values is roughly [0,0.62], so scale by 820 to
  // get 9-bit values
...
float3 _UnchartedCurve(float3 color) {
  ...
  return saturate(((color * (A * color + C * B) + D * E) /
                   (color * (A * color + B) + D * F)) -
                  (E / F));
}
...
float3 HdrToLinear_Aces(float3 color) {
  ...
  return saturate((color * (A * color + B)) *
                  Math::PositiveRcp(color * (C * color + D) + E));
}
...
float4 BlendDifference(float4 a, float4 b) { return abs(a - b); }

float4 BlendScreen(float4 a, float4 b) { return a + b * (1.0 - a); }
...
float3 ColorizeLinear(float x) {
  ...
  if (x < 0.25)
    color =
        lerp(float3(0, 0, 0), float3(0, 0, 1), Math::SmoothStep(0.00, 0.25, x));
  else if (x < 0.50)
    color =
        lerp(float3(0, 0, 1), float3(0, 1, 0), Math::SmoothStep(0.25, 0.50, x));
  else if (x < 0.75)
    color =
        lerp(float3(0, 1, 0), float3(1, 1, 0), Math::SmoothStep(0.50, 0.75, x));
  else
    color =
        lerp(float3(1, 1, 0), float3(1, 0, 0), Math::SmoothStep(0.75, 1.00, x));
...
float3 ColorizeZucconi(float x) {
  // Original solution converts visible wavelengths of light (400-700 nm)
  // (represented as x = [0; 1]) to RGB colors
  x = saturate(x) * 0.85;
...
// Unsigned packing
uint RgbaToUint(float4 c, compiletime const uint Rbits,
                compiletime const uint Gbits = 0,
                compiletime const uint Bbits = 0,
                compiletime const uint Abits = 0) {
...
// Unsigned unpacking
float4 UintToRgba(uint p, compiletime const uint Rbits,
                  compiletime const uint Gbits = 0,
                  compiletime const uint Bbits = 0,
                  compiletime const uint Abits = 0) {
  const uint Rmask = (1u << Rbits) - 1u;
...
uint Rg16fToUint(float2 c) {
  return (f32tof16(c.y) << 16) |
         (f32tof16(c.x) &
          0xFFFF); // TODO: 0xFFFF is a WAR for NV VK compiler bug (<= 461.xx)
}
...
// [Cigolle 2014, "A Survey of Efficient Representations for Independent Unit
// Vectors"] Error ( deg )
//                    Mean      Max
...
float3 DecodeUnitVector(float2 p, compiletime const bool bSigned = false,
                        compiletime const bool bNormalize = true) {
...
namespace Filtering {
float GetModifiedRoughnessFromNormalVariance(
    float linearRoughness, float3 nonNormalizedAverageNormal) {
...
// Mipmap level
float GetMipmapLevel(float duvdxMulTexSizeSq, float duvdyMulTexSizeSq,
                     compiletime const float maxAnisotropy = 1.0) {
...
float ApplyBilinearFilter(float s00, float s10, float s01, float s11,
                          Bilinear f) {
  return lerp(lerp(s00, s10, f.weights.x), lerp(s01, s11, f.weights.x),
              f.weights.y);
}

float2 ApplyBilinearFilter(float2 s00, float2 s10, float2 s01, float2 s11,
                           Bilinear f) {
  return lerp(lerp(s00, s10, f.weights.x), lerp(s01, s11, f.weights.x),
              f.weights.y);
}

float3 ApplyBilinearFilter(float3 s00, float3 s10, float3 s01, float3 s11,
                           Bilinear f) {
  return lerp(lerp(s00, s10, f.weights.x), lerp(s01, s11, f.weights.x),
              f.weights.y);
}

float4 ApplyBilinearFilter(float4 s00, float4 s10, float4 s01, float4 s11,
                           Bilinear f) {
  return lerp(lerp(s00, s10, f.weights.x), lerp(s01, s11, f.weights.x),
              f.weights.y);
}
...
// Normalization avoids dividing by small numbers to mitigate potential
// imprecision problems
#define _ApplyBilinearCustomWeights(s00, s10, s01, s11, w, normalize)          \
  ((s00 * w.x + s10 * w.y + s01 * w.z + s11 * w.w) *                           \
   (normalize ? (dot(w, 1.0) < 0.0001 ? 0.0 : rcp(dot(w, 1.0))) : 1.0))

float ApplyBilinearCustomWeights(float s00, float s10, float s01, float s11,
                                 float4 w,
                                 compiletime const bool normalize = true) {
  return _ApplyBilinearCustomWeights(s00, s10, s01, s11, w, normalize);
}

float2 ApplyBilinearCustomWeights(float2 s00, float2 s10, float2 s01,
                                  float2 s11, float4 w,
                                  compiletime const bool normalize = true) {
  return _ApplyBilinearCustomWeights(s00, s10, s01, s11, w, normalize);
}

float3 ApplyBilinearCustomWeights(float3 s00, float3 s10, float3 s01,
                                  float3 s11, float4 w,
                                  compiletime const bool normalize = true) {
  return _ApplyBilinearCustomWeights(s00, s10, s01, s11, w, normalize);
}

float4 ApplyBilinearCustomWeights(float4 s00, float4 s10, float4 s01,
                                  float4 s11, float4 w,
                                  compiletime const bool normalize = true) {
  return _ApplyBilinearCustomWeights(s00, s10, s01, s11, w, normalize);
}
...
float4 ApplyCatmullRomFilterNoCorners(CatmullRom filter, float4 s10, float4 s20,
                                      float4 s01, float4 s11, float4 s21,
                                      float4 s31, float4 s02, float4 s12,
                                      float4 s22, float4 s32, float4 s13,
                                      float4 s23) {
  /*
...
float ApplyCatmullRomFilterNoCorners(CatmullRom filter, float s10, float s20,
                                     float s01, float s11, float s21, float s31,
                                     float s02, float s12, float s22, float s32,
                                     float s13, float s23) {
  /*
  s00, s30, s03, s33 - are not used
...
uint Bayer4x4ui(uint2 samplePos, uint frameIndex,
                compiletime const uint mode = STL_BAYER_DEFAULT) {
  uint2 samplePosWrap = samplePos & 3;
  uint a = 2068378560 * (1 - (samplePosWrap.x >> 1)) +
           1500172770 * (samplePosWrap.x >> 1);
  uint b = (samplePosWrap.y + ((samplePosWrap.x & 1) << 2)) << 2;

  uint sampleOffset = mode == STL_BAYER_REVERSEBITS
                          ? Math::ReverseBits4(frameIndex)
                          : frameIndex;
...
// NOTE: Hash( 0 ) = 0! If that's a problem do "Hash32( x + constant )".
// HashCombine does something similar already
uint Hash(uint x) {
// This little gem is from https://nullprogram.com/blog/2018/07/31/,
// "Prospecting for Hash Functions" by Chris Wellons
...
void Initialize(uint linearIndex, uint frameIndex, uint spinNum = 16) {
  ...
  uint s = 0;
  [unroll] for (uint n = 0; n < spinNum; n++) {
    s += 0x9E3779B9;
    g_TeaSeed.x += ((g_TeaSeed.y << 4) + 0xA341316C) ^ (g_TeaSeed.y + s) ^
                   ((g_TeaSeed.y >> 5) + 0xC8013EA4);
    g_TeaSeed.y += ((g_TeaSeed.x << 4) + 0xAD90777D) ^ (g_TeaSeed.x + s) ^
                   ((g_TeaSeed.x >> 5) + 0x7E95761E);
  }
}
...
void Initialize(uint linearIndex, uint frameIndex) {
  g_HashSeed = Sequence::HashCombine(Sequence::Hash(frameIndex + 0x035F9F29),
                                     linearIndex);
}
...
// "linearRoughness" ( aka "perceptual roughness", aka "artistic roughness" ) =
// sqrt( roughness ) G1 = G1( V, m ) is % visible in one direction G2 = G2( L,
// V, m ) is % visible in two directions ( in practice, derived from G1 ) G2(
// uncorellated ) = G1( L, m ) * G1( V, m ) Specular BRDF = F * D * G2 / ( 4.0 *
// NoV * NoL )
...
float Pow5(float x) { return Math::Pow01(1.0 - x, 5.0); }

void ConvertBaseColorMetalnessToAlbedoRf0(float3 baseColor, float metalness,
                                          out float3 albedo, out float3 Rf0) {
  // TODO: ideally, STL_RF0_DIELECTRICS needs to be replaced with reflectance
  // "STL_RF0_DIELECTRICS = 0.16 * reflectance * reflectance" see
  // https://google.github.io/filament/Filament.html#toc4.8 see
...
float FresnelTerm_Shadowing(float3 Rf0) {
  // UE4: anything less than 2% is physically impossible and is instead
...
float DiffuseTerm_Lambert(float linearRoughness, float NoL, float NoV,
                          float VoH) {
  float d = 1.0;
...
float DiffuseTerm_Burley(float linearRoughness, float NoL, float NoV,
                         float VoH) {
...
// [Gotanda 2012, "Beyond a Simple Physically Based Blinn-Phong Model in
// Real-Time"]
float DiffuseTerm_OrenNayar(float linearRoughness, float NoL, float NoV,
                            float VoH) {
  float m = linearRoughness * linearRoughness;
  float m2 = m * m;
...
//======================================================================================================================
// Distribution terms - how the microfacet normal distributed around a given
// direction
//======================================================================================================================
...
// GGX / Trowbridge-Reitz, [Walter et al. 2007, "Microfacet models for
...
// Generalized Trowbridge-Reitz, [Burley 2012, "Physically-Based Shading at
// Disney"]
...
//======================================================================================================================
// Geometry terms - how much the microfacet is blocked by other microfacet
// G(mod) = G / ( 4.0 * NoV * NoL ): BRDF = F * D * G / ( 4 * NoV * NoL ) =>
// BRDF = F * D * Gmod
//======================================================================================================================

float GeometryTermMod_Implicit(float linearRoughness, float NoL, float NoV,
                               float VoH, float NoH) {
  return 0.25;
}

// [Neumann 1999, "Compact metallic reflectance models"]
float GeometryTermMod_Neumann(float linearRoughness, float NoL, float NoV,
                              float VoH, float NoH) {
  return 0.25 * Math::PositiveRcp(max(NoL, NoV));
}

// [Schlick 1994, "An Inexpensive BRDF Model for Physically-Based Rendering"]
float GeometryTermMod_Schlick(float linearRoughness, float NoL, float NoV,
                              float VoH, float NoH) {
  float m = linearRoughness * linearRoughness;
...
float GeometryTermMod_SmithCorrelated(float linearRoughness, float NoL,
                                      float NoV, float VoH, float NoH) {
  float m = linearRoughness * linearRoughness;
  float m2 = m * m;
...
// Smith term for GGX modified by Disney to be less "hot" for small roughness
// values [Burley 2012, "Physically-Based Shading at Disney"] Known as "G2 = G1(
// NoL ) * G1( NoV )"
float GeometryTermMod_SmithUncorrelated(float linearRoughness, float NoL,
                                        float NoV, float VoH, float NoH) {
  float m = linearRoughness * linearRoughness;
  float m2 = m * m;
...
float GeometryTermMod_CookTorrance(float linearRoughness, float NoL, float NoV,
                                   float VoH, float NoH) {
  float k = 2.0 * NoH / VoH;
...
float GeometryTermMod(float linearRoughness, float NoL, float NoV, float VoH,
                      float NoH) {
...
//======================================================================================================================
// Fresnel terms - the amount of light that reflects from a mirror surface given
// its index of refraction
//======================================================================================================================

float3 FresnelTerm_None(float3 Rf0, float VoNH) { return Rf0; }
...
float FresnelTerm_Dielectric(float eta, float VoN) {
  ...
  // Cosine of angle between negative normal and transmitted direction ( 0 for
...
// Approximate Models For Physically Based Rendering by Angelo Pesce and Michael
// Iwanicki http://miciwan.com/SIGGRAPH2015/course_notes_wip.pdf
...
// Shlick's approximation for Ross BRDF - makes Fresnel converge to less
// than 1.0 when NoV is low
...
float3 EnvironmentTerm_Ross(float3 Rf0, float NoV, float linearRoughness) {
  ...
  float f = Math::Pow01(1.0 - NoV, 5.0 * exp(-2.69 * m)) /
            (1.0 + 22.7 * Math::Pow01(m, 1.5));
...
// "Ray Tracing Gems", Chapter 32, Equation 4 - the approximation assumes GGX
// VNDF and Schlick's approximation
float3 EnvironmentTerm_Rtg(float3 Rf0, float NoV, float linearRoughness) {
  ...
  float3x3 M2 = float3x3(1.0, 2.92338, 59.4188, 20.3225, -27.0302, 222.592,
                         121.563, 626.13, 316.627);

  float2x2 M3 = float2x2(0.0365463, 3.32707, 9.0632, -9.04756);
  float3x3 M4 = float3x3(1.0, 3.59685, -1.36772, 9.04401, -16.3174, 9.22949,
                         5.56589, 19.7886, -20.2123);

  float bias =
      dot(mul(M1, X.xy), Y.xy) * Math::PositiveRcp(dot(mul(M2, X.xyw), Y.xyw));
  float scale =
      dot(mul(M3, X.xy), Y.xy) * Math::PositiveRcp(dot(mul(M4, X.xzw), Y.xyw));
...
void DirectLighting(float3 N, float3 L, float3 V, float3 Rf0,
                    float linearRoughness, out float3 Cdiff, out float3 Cspec) {
...
float GetSpecularLobeHalfAngle(float linearRoughness,
...
float GetSpecularLobeTanHalfAngle(float linearRoughness,
                                  float percentOfVolume = 0.75) {
  float m = linearRoughness * linearRoughness;
...
float3 CorrectDirectionToInfiniteSource(float3 N, float3 L, float3 V,
                                        float tanOfAngularSize) {
  float3 R = reflect(-V, N);
  float3 centerToRay = L - dot(L, R) * R;
  float3 closestPoint =
      centerToRay * saturate(tanOfAngularSize *
                             Math::Rsqrt(Math::LengthSquared(centerToRay)));
...
float GetSpecularDominantFactor(
    float NoV, float linearRoughness,
    compiletime const uint mode = STL_SPECULAR_DOMINANT_DIRECTION_DEFAULT) {
  ...
  if (mode == STL_SPECULAR_DOMINANT_DIRECTION_G2) {
    ...
    dominantFactor =
        0.298475 * NoV * log(39.4115 - 39.0029 * linearRoughness) +
        (0.385503 - 0.385503 * NoV) * log(13.1567 - 12.2848 * linearRoughness);
  else {
...
float3 GetSpecularDominantDirectionWithFactor(float3 N, float3 V,
                                              float dominantFactor) {
  float3 R = reflect(-V, N);
...
float4 GetSpecularDominantDirection(
    float3 N, float3 V, float linearRoughness,
    compiletime const uint mode = STL_SPECULAR_DOMINANT_DIRECTION_DEFAULT) {
  float NoV = abs(dot(N, V));
  float dominantFactor = GetSpecularDominantFactor(NoV, linearRoughness, mode);

  return float4(GetSpecularDominantDirectionWithFactor(N, V, dominantFactor),
                dominantFactor);
}
...
namespace Uniform {
float GetPDF() { return 1.0 / Math::Pi(2.0); }
...
namespace Cosine {
float GetPDF(float NoL = 1.0) // default can be useful to handle NoL cancelation
                              // ( PDF's NoL cancels throughput's NoL )
{
...
float GetPDF(float NoV, float NoH, float linearRoughness) {
  ...
  */
  ...
  float m2 = m * m;
  float G1mod =
      Math::PositiveRcp(NoV + Math::Sqrt01((NoV - m2 * NoV) * NoV +
                                           m2)); // see GeometryTerm_Smith
...
float3 GetRay(float2 rnd, float2 linearRoughness, float3 Vlocal,
              float trimFactor = 1.0) {
  /*
  Trimming:
      0.0  - dominant direction
      1.0  - full lobe
      0.95 - allows to cut off samples with very low probabilities ( good for
  denoising )
  */
  rnd.y *= trimFactor;
...
float3 ExtractColor(SH1 sh) { return Color::YCoCgToLinear(sh.c0_chroma); }
...
#define CHAR(a1, a2, a3, a4, a5, b1, b2, b3, b4, b5, c1, c2, c3, c4, c5, d1,   \
             d2, d3, d4, d5, e1, e2, e3, e4, e5, f1, f2, f3, f4, f5)           \
  ((a1 << 0) | (a2 << 1) | (a3 << 2) | (a4 << 3) | (a5 << 4) | (b1 << 5) |     \
   (b2 << 6) | (b3 << 7) | (b4 << 8) | (b5 << 9) | (c1 << 10) | (c2 << 11) |   \
   (c3 << 12) | (c4 << 13) | (c5 << 14) | (d1 << 15) | (d2 << 16) |            \
   (d3 << 17) | (d4 << 18) | (d5 << 19) | (e1 << 20) | (e2 << 21) |            \
   (e3 << 22) | (e4 << 23) | (e5 << 24) | (f1 << 25) | (f2 << 26) |            \
   (f3 << 27) | (f4 << 28) | (f5 << 29))

static const uint font[] = {
    CHAR(_, X, X, X, _, X, _, _, _, X, X, _, _, _, X, X, _, _, _, X, X, _, _, _,
         X, _, X, X, X, _),

    CHAR(_, _, _, _, X, _, _, _, X, X, _, _, _, _, X, _, _, _, _, X, _, _, _, _,
         X, _, _, _, _, X),

    CHAR(X, X, X, X, _, _, _, _, _, X, X, X, X, X, X, X, _, _, _, _, X, _, _, _,
         _, X, X, X, X, X),

    CHAR(X, X, X, X, _, _, _, _, _, X, _, X, X, X, X, _, _, _, _, X, _, _, _, _,
         X, X, X, X, X, _),

    CHAR(X, _, _, _, X, X, _, _, _, X, X, X, X, X, X, _, _, _, _, X, _, _, _, _,
         X, _, _, _, _, X),

    CHAR(X, X, X, X, _, X, _, _, _, _, X, X, X, X, X, _, _, _, _, X, _, _, _, _,
         X, X, X, X, X, _),

    CHAR(_, X, X, X, X, X, _, _, _, _, X, X, X, X, X, X, _, _, _, X, X, _, _, _,
         X, X, X, X, X, _),

    CHAR(_, X, X, X, _, _, _, _, _, X, _, _, _, _, X, _, _, _, _, X, _, _, _, _,
         X, _, _, _, _, X),

    CHAR(_, X, X, X, _, X, _, _, _, X, X, X, X, X, X, X, _, _, _, X, X, _, _, _,
         X, _, X, X, X, _),

    CHAR(_, X, X, X, _, X, _, _, _, X, X, X, X, X, X, _, _, _, _, X, _, _, _, _,
         X, X, X, X, X, _),

    CHAR(_, _, _, _, _, _, _, _, _, _, X, X, X, X, X, _, _, _, _, _, _, _, _, _,
         _, _, _, _, _, _),

    CHAR(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _,
         _, _, _, X, _, _),

    CHAR(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _,
         _, _, _, _, _, _),

    CHAR(_, _, X, _, _, _, X, _, X, _, _, _, X, _, _, _, X, _, X, _, X, _, _, X,
         _, _, X, X, _, X),

    CHAR(_, _, X, X, X, _, X, _, _, X, X, _, _, _, X, X, X, X, X, X, X, _, _, _,
         X, X, _, _, _, X),

    CHAR(X, X, X, X, _, X, _, _, _, X, X, X, X, X, _, X, _, _, _, X, X, _, _, _,
         X, X, X, X, X, _),

    CHAR(_, X, X, X, X, X, _, _, _, _, X, _, _, _, _, X, _, _, _, _, X, _, _, _,
         _, _, X, X, X, X),

    CHAR(X, X, X, X, _, X, _, _, _, X, X, _, _, _, X, X, _, _, _, X, X, _, _, _,
         X, X, X, X, X, _),

    CHAR(X, X, X, X, X, X, _, _, _, _, X, X, X, X, _, X, _, _, _, _, X, _, _, _,
         _, X, X, X, X, X),

    CHAR(X, X, X, X, X, X, _, _, _, _, X, X, X, X, _, X, _, _, _, _, X, _, _, _,
         _, X, _, _, _, _),

    CHAR(_, X, X, X, X, X, _, _, _, _, X, _, _, _, _, X, _, _, X, X, X, _, _, _,
         X, _, X, X, X, X),

    CHAR(X, _, _, _, X, X, _, _, _, X, X, X, X, X, X, X, _, _, _, X, X, _, _, _,
         X, X, _, _, _, X),

    CHAR(_, X, X, X, _, _, _, X, _, _, _, _, X, _, _, _, _, X, _, _, _, _, X, _,
         _, _, X, X, X, _),

    CHAR(_, _, _, _, X, _, _, _, _, X, _, _, _, _, X, _, _, _, _, X, X, _, _, _,
         X, _, X, X, X, _),

    CHAR(X, _, _, _, X, X, _, _, X, _, X, X, X, _, _, X, _, _, X, _, X, _, _, _,
         X, X, _, _, _, X),

    CHAR(X, _, _, _, _, X, _, _, _, _, X, _, _, _, _, X, _, _, _, _, X, _, _, _,
         _, X, X, X, X, X),

    CHAR(X, _, _, _, X, X, X, _, X, X, X, _, X, _, X, X, _, _, _, X, X, _, _, _,
         X, X, _, _, _, X),

    CHAR(X, _, _, _, X, X, X, _, _, X, X, _, X, _, X, X, _, _, X, X, X, _, _, _,
         X, X, _, _, _, X),

    CHAR(_, X, X, X, _, X, _, _, _, X, X, _, _, _, X, X, _, _, _, X, X, _, _, _,
         X, _, X, X, X, _),

    CHAR(X, X, X, X, _, X, _, _, _, X, X, _, _, _, X, X, X, X, X, _, X, _, _, _,
         _, X, _, _, _, _),

    CHAR(_, X, X, X, _, X, _, _, _, X, X, _, _, _, X, X, _, X, _, X, X, _, _, X,
         _, _, X, X, _, X),

    CHAR(X, X, X, X, _, X, _, _, _, X, X, _, _, _, X, X, X, X, X, _, X, _, _, X,
         _, X, _, _, _, X),

    CHAR(_, X, X, X, X, X, _, _, _, _, _, X, X, X, _, _, _, _, _, X, _, _, _, _,
         X, X, X, X, X, _),

    CHAR(X, X, X, X, X, _, _, X, _, _, _, _, X, _, _, _, _, X, _, _, _, _, X, _,
         _, _, _, X, _, _),

    CHAR(X, _, _, _, X, X, _, _, _, X, X, _, _, _, X, X, _, _, _, X, X, _, _, _,
         X, _, X, X, X, _),

    CHAR(X, _, _, _, X, X, _, _, _, X, X, _, _, _, X, X, _, _, _, X, _, X, _, X,
         _, _, _, X, _, _),

    CHAR(X, _, _, _, X, X, _, _, _, X, X, _, X, _, X, X, _, X, _, X, X, X, X, X,
         X, _, X, _, X, _),

    CHAR(X, _, _, _, X, _, X, _, X, _, _, _, X, _, _, _, X, _, X, _, X, _, _, _,
         X, X, _, _, _, X),

    CHAR(X, _, _, _, X, _, X, _, X, _, _, X, _, X, _, _, _, X, _, _, _, _, X, _,
         _, _, _, X, _, _),

    CHAR(X, X, X, X, X, _, _, _, X, _, _, _, X, _, _, _, X, _, _, _, X, _, _, _,
         _, X, X, X, X, X),
};
...
#define _RenderChar(ch)                                                        \
  if (all(int2(state.xy) >= -int(state.z) &&                                   \
          int2(state.xy) < int2(CHAR_W + 1, CHAR_H + 1) * int(state.z))) {     \
    state.w = BACKGROUND;                                                      \
...
#define _RenderChar(ch)                                                        \
  [flatten] if (all(state.xy < uint2(CHAR_W, CHAR_H) * state.z)) {             \
    uint2 p = state.xy / state.z;                                              \
    uint bits = font[ch];                                                      \
    uint bit = bits & (1u << (p.y * CHAR_W + p.x));                            \
    state.w = bit ? FOREGROUND : BACKGROUND;                                   \
...
bool IsBackground(uint4 state) { return state.w == BACKGROUND; }

bool IsForeground(uint4 state) { return state.w == FOREGROUND; }

void NextChar(inout uint4 state) { state.x -= (CHAR_W + 1) * state.z; }

void NextDigit(inout uint4 state) { state.x += (CHAR_W + 1) * state.z; }
...
void Print_f(float x, inout uint4 state) {
  ...
  // Zeros after dot
  [unroll] for (uint n = 10; n < STL_TEXT_DIGIT_FORMAT; n *= 10) {
    if (f < n) {
      NextDigit(state);
      _RenderChar(Char_0);
    }
