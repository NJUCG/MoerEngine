...
[numthreads(256, 1, 1)] void main(uint3 group_id
    ...
    //lid和bit对应的flag
    uint64_t bit = lID % BITS;
    const uint64_t flags_bit = uint64_t(1UL) << bit;
    ...
    for (uint index = 0; index < g_num_blocks_per_workgroup; index++) {
        ...
        if (elementId < g_num_instances) {
            ...
            uint64_t shift_bit = uint64_t(element_in >> g_shift);
            binID      = uint(shift_bit) & uint(RADIX_SORT_BINS - 1);
            // offset for group
