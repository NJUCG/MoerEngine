...
#define PCSS_DYNAMIC_DEPTH_BIAS 1
...
DEFINE_CALCULATE_PCF(Point)
DEFINE_CALCULATE_PCF(Dir)

bool IsDirectionalLight_Point() {
    return false;
}
bool IsDirectionalLight_Dir() {
    return true;
}

#define DEFINE_FIND_BLOCKER(SUFFIX)                                                                          \
    BlockerStats FindBlocker##SUFFIX(ShadowContext ctx, float search_radius_uv) {                            \
        BlockerStats stats;                                                                                  \
        stats.blockerCase = 1;                                                                               \
        stats.avgDepth    = 0;                                                                               \
        stats.numBlockers = 0;                                                                               \
        ...
        [unroll] for (int i = 0; i < PCSS_SEARCH_SAMPLES; ++i) {                                             \
            ...
            float  linear_occluder_depth = LinearizeShadowDepth##SUFFIX(ctx, raw_occluder_depth);            \
                                                                                                             \
            if (IsShadowed##SUFFIX(linear_occluder_depth, GetFragmentDepth##SUFFIX(ctx), dynamicBias)) {     \
...
DEFINE_FIND_BLOCKER(Point)
DEFINE_FIND_BLOCKER(Dir)

#define DEFINE_CALCULATE_PCSS(SUFFIX)                                                                                       \
    float CalculatePcss##SUFFIX(ShadowContext ctx) {                                                                        \
        /* Blocker Search */                                                                                                \
        float        search_radius_uv = GetSearchRadiusUV##SUFFIX(ctx);                                                     \
        BlockerStats stats            = FindBlocker##SUFFIX(ctx, search_radius_uv);                                         \
                                                                                                                            \
        if (PCSS_DEBUG_EARLY_RETURN) {                                                                                      \
        ...
        if (PCSS_ANTI_ALIASING_METHOD == 2) {                                                                               \
            ...
            pcf_matrix = GenerateDirectionalScale2x2Matrix(major_axis, elliptical_factor);                                  \
                                                                                                                            ...
                                                                                                                            \
        float shadow_visibility = CalculatePcf##SUFFIX(ctx, penumbra_uv, pcf_matrix);                                       \
                                                                                                                            \
        if (PCSS_ENABLE_POST_PCF_SHARPENING) {                                                                              \
            const float AverageSampleDistance = sqrt(1.0 / (float(PCSS_SEARCH_SAMPLES) * 3.14159265359));                   \
            ...
            float final_sharpness_factor =                                                                                  \
                lerp(1.0, clamp(radius_ratio, 1.0, MaxSharpnessFactor), sharpeness_fading);                                 \
                                                                                                                            \
