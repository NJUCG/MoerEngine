...
[numthreads(Moer::TONEMAPPING_HISTOGRAM_GROUP_X, Moer::TONEMAPPING_HISTOGRAM_GROUP_Y, 1)]
void main(uint gid : SV_GroupIndex, uint2 thread_id : SV_DispatchThreadId) {
    ...
    if (is_valid) {
        ...
        float log2lum = saturate(
            log2(luminance) * param.log2lum_saturate_scale
