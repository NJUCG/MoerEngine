...
struct AoOutput {
    float  ambient_only : SV_Target0;
    float2 camera_motion_vector : SV_Target1;
};
...
float2 GetCameraMotionVector(float2 uv) {
    ...
    float depth = TextureHandle(param.depth_tex).SampleLevel<float>(uv);
    if (depth < Epsilon) {
        return float2(0.0, 0.0);
    }

    float3 world_pos_vec3 = WorldPosFromDepth(depth, uv, param.clip2world);
