...
[[vk::binding(3, DI_BINDING_SLOT)]] RWStructuredBuffer<Moer::DI::PackedReservoir> light_reservoirs;
...
[[vk::binding(9, DI_BINDING_SLOT)]] RWTexture2D<float4> rw_diffuse_lighting_prev;
...
float3 SpecularTerm(float3 _v, float3 _l, float3 _n, float _roughness, float3 _f0, out float3 _fresnel) {
    float3 h   = normalize(_v + _l);
...
struct Surface {
    ...
    float GetDiffuseProbability() {
        ...
        float  diffuse_weight  = STL::Color::Luminance(diffuse_albedo * (1.f - fresnel));
    ...
    bool GetBrdfSample(out float3 _dir, inout RandomState _rng) {
        ...
        if (rnd.z < diffuse_prob) {
            ...
            float3 h  = STL::ImportanceSampling::VNDF::GetRay(rnd.xy, float2(roughness, roughness), ve);
            h         = normalize(h);
    ...
    float GetBrdfPdf(float3 _dir) {
        ...
        float specular_pdf = STL::ImportanceSampling::VNDF::GetPDF(nov, noh, roughness);

        return cos_theta > 0.f ? lerp(specular_pdf, diffuse_pdf, diffuse_prob) : 0.f;
    }
    ...
    float GetLightSampleTargetPdf(LightSample _sample) {
        ...
        float3 reflect_radiance = ((1.f - fresnel) * d * diffuse_albedo + s) * _sample.radiance;
    ...
    void GetLightDirAndDist(LightSample _light, out float3 _dir, out float _distance) {
    ...
    void EvalBrdf(float3 _sample_pos, out float3 _diffuse, out float3 _specular) {
        ...
        float3 fresnel = 0.f;
        _specular      = SpecularTerm(v, normalize(l), n, roughness, specular_f0, fresnel);
    ...
    bool HasSimilarMaterial(Surface _other) {
        ...
        if (!Math::CompareDifferance(roughness, _other.roughness, roughness_threshold))
            return false;

        if (abs(STL::Color::Luminance(diffuse_albedo) - STL::Color::Luminance(_other.diffuse_albedo)) >
            albedo_threshold)
            return false;

        if (abs(STL::Color::Luminance(specular_f0) - STL::Color::Luminance(_other.specular_f0)) >
...
int GetMappedLightIndex(int _cur_light_idx) {
    ArrayBuffer light_idx_buf = ArrayBuffer(resample_params.bindless_handles.light_index);
...
Surface GetGBufferSurface(int2 _pixel_pos, const bool _prev_frame = false) {
    ...
    if (_prev_frame) {
        ...
        gbuffer_specular_roughness =
            TextureHandle(resample_params.bindless_handles.gbuffer_prev_specular_roughness);
        ...
        gbuffer_specular_roughness =
            TextureHandle(resample_params.bindless_handles.gbuffer_specular_roughness);
    }
...
float EvalEnvMapPdf(float3 _dir) {
    ...
    TextureHandle    env_map_pdf     = TextureHandle(resample_params.bindless_handles.env_pdf);
...
float EvalLocalLightSrcPdf(uint _light_idx) {
    ...
    Texture2D<float> local_light_pdf_tex =
        TextureHandle(resample_params.bindless_handles.local_light_pdf).GetTexture2D<float>();
...
PolymorphicLightInfo LoadLightInfo(uint _idx) {
    ArrayBuffer light_buffer = ArrayBuffer(resample_params.bindless_handles.poly_light_data);
...
#if USE_RAYQUERY
    RayQuery<RAY_FLAG_SKIP_PROCEDURAL_PRIMITIVES | RAY_FLAG_ACCEPT_FIRST_HIT_AND_END_SEARCH> ray_query;
...
float3 GetFinalVisibility(RaytracingAccelerationStructure _tlas, Surface _surface, float3 _sample_pos) {
    RayDesc ray = _surface.SetupVisibilityRay(_sample_pos);
    ...
    while (ray_query.Proceed()) {
        if (ray_query.CandidateType() == CANDIDATE_NON_OPAQUE_TRIANGLE) {
            if (EvalTransparentMaterial(
                    ray_query.CandidateInstanceID(),
