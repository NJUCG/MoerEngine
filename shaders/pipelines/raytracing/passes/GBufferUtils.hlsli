...
float3 GetMotion(
    Moer::ViewParam _view,
    Moer::ViewParam _prev_view,
    float4x4        _model2world,
    float4x4        _prev_model2world,
    float3          _model_pos,
    float3          _model_pos_prev,
    out float       _clip_depth,
    out float       _view_depth
) {
    float3 world_pos      = mul(_model2world, float4(_model_pos, 1.0f)).xyz;
    float3 world_pos_prev = mul(_prev_model2world, float4(_model_pos_prev, 1.0f)).xyz;

    float4 clip_pos = mul(_view.world2clip, float4(world_pos, 1.0f));

    float4 clip_pos_prev = mul(_prev_view.world2clip, float4(world_pos_prev, 1.0f));
...
float2 GetEnvMotion(Moer::ViewParam _view, Moer::ViewParam _prev_view, float2 _pixel_pos) {
    float4 clip_pos;
    ...
    float4 world_pos = mul(_view.clip2world, clip_pos);
    world_pos.xyz /= world_pos.w;
    float4 prev_clip_pos = mul(_prev_view.world2clip, float4(world_pos.xyz, 1.0f));
...
float3
MotionToPixelSpace(Moer::ViewParam _view, Moer::ViewParam _prev_view, int2 _pixel_pos, float3 _motion) {
...
float3 GetPrevWorldPos(Moer::ViewParam _prev_view, int2 _pixel_pos, float _view_depth, float3 _motion) {
