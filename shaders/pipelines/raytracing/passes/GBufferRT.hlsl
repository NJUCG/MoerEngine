    ...
    while (ray_query.Proceed()) {
        if (ray_query.CandidateType() == CANDIDATE_NON_OPAQUE_TRIANGLE) {
            if (ProcessAnyHit(
                    ...
                    ray_query.CandidatePrimitiveIndex(), // 返回的应该是三角形索引，而非index(uint)索引
...
// TraceRay
    ...
    if (payload.instance_id != ~0u) {
        ...
        float4x4 model2world = geom.instance.world_transform;

        // 计算纹理梯度（用于各向异性采样）
        RayDesc r0 = ray;
        RayDesc r1 = Moer::SetupPrimaryRay(pixel_pos + uint2(1, 0), gbuffer_constants.main_view);
        RayDesc r2 = Moer::SetupPrimaryRay(pixel_pos + uint2(0, 1), gbuffer_constants.main_view);

        // 三角形顶点变换到世界空间
        float3 pos_ws[3];
        pos_ws[0] = mul(model2world, float4(geom.vtx_positions[0], 1.f)).xyz;
        pos_ws[1] = mul(model2world, float4(geom.vtx_positions[1], 1.f)).xyz;
        pos_ws[2] = mul(model2world, float4(geom.vtx_positions[2], 1.f)).xyz;

        float3 bary0 =
            Moer::RayIntersectBarycentrics(r0.Origin, r0.Direction, pos_ws[0], pos_ws[1], pos_ws[2]);
        float3 baryx =
            Moer::RayIntersectBarycentrics(r1.Origin, r1.Direction, pos_ws[0], pos_ws[1], pos_ws[2]);
        float3 baryy =
            Moer::RayIntersectBarycentrics(r2.Origin, r2.Direction, pos_ws[0], pos_ws[1], pos_ws[2]);
        ...
        // 采样材质
        Moer::MaterialSample mat_sample =
            Moer::SampleGeometryMaterial(geom, tex_grad_x, tex_grad_y, -1.f, Moer::EMA_All);

        // 计算 motion vector（用于去噪）
        float  clip_depth = 0.f;
        float  view_depth = 0.f;
        float3 motion     = Moer::GetMotion(
            gbuffer_constants.main_view,
            gbuffer_constants.prev_view,
            model2world,
            model2world, // TODO: 暂无 prev_model2world，传入当前帧变换
            geom.model_pos,
            geom.model_pos_prev,
            clip_depth,
            view_depth
