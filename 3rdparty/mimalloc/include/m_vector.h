...
public:
    ...
    m_vector(size_t n) : _start(nullptr), _finish(nullptr), _endOfStorage(nullptr) {
        reserve(n);
    }

    m_vector(size_t n, const T& value) : _start(nullptr), _finish(nullptr), _endOfStorage(nullptr) {
        reserve(n);
        for (int i = 0; i < n; ++i)
            push_back(value);
    }
    ...
    m_vector(std::initializer_list<T> list) : _start(nullptr), _finish(nullptr), _endOfStorage(nullptr) {
        reserve(list.size());
        iterator pos = _start;
        for (auto l : list)
    ...
    void emplace_back(Args&&... args) {
        ...
        allocator.construct(_finish, std::forward<Args>(args)...);
        ++_finish;
    }
    ...
    void reserve(size_t n) {
        if (n > capacity()) {
            ...
            if (_start) {
                for (int i = 0; i < s; ++i)
                    allocator.construct(pos++, _start[i]);
                clear();
