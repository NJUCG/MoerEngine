...
#define NRD_INTEGRATION_ABORT_ON_FAILURE(result) \
    if ((result) != nri::Result::SUCCESS) NRD_INTEGRATION_ASSERT(false, "Abort on failure!")
...
namespace nrd {
    ...
    public:
        ...
        inline ~Integration() { NRD_INTEGRATION_ASSERT(m_NRI == nullptr, "m_NRI must be NULL at this point!"); }
        ...
        // Should called only when the resource dimensions change
        void RecreateResources(uint16_t resourceWidth, uint16_t resourceHeight);

        // Helpers
        inline double GetTotalMemoryUsageInMb() const { return double(m_PermanentPoolSize + m_TransientPoolSize) / (1024.0 * 1024.0); }

        inline double GetPersistentMemoryUsageInMb() const { return double(m_PermanentPoolSize) / (1024.0 * 1024.0); }

        inline double GetAliasableMemoryUsageInMb() const { return double(m_TransientPoolSize) / (1024.0 * 1024.0); }
