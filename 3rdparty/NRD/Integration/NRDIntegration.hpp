...
namespace nrd {
    ...
    static inline uint16_t DivideUp(uint32_t x, uint16_t y) { return uint16_t((x + y - 1) / y); }
    ...
    void Integration::RecreateResources(uint16_t resourceWidth, uint16_t resourceHeight) {
        // Free all previous resolution textures
        NRD_INTEGRATION_ASSERT(m_Instance, "NRI Instance is already destroyed! Did you forget to call 'Initialize'?");
        ...
        // Create new resolution textures
        const InstanceDesc& instanceDesc = GetInstanceDesc(*m_Instance);
        ...
        // Update buffer&texture allocations
