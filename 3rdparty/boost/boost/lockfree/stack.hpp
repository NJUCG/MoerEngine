...
public:
    ...
    size_t consume_all_atomic_reversed(Functor const & f)
    {
        ...
        for (;;) {
            ...
            tagged_node_handle new_tos(pool.null_handle(), old_tos.get_next_stat());
...
private:
    ...
    BOOST_DELETED_FUNCTION(closable_stack(closable_stack const&))
        BOOST_DELETED_FUNCTION(closable_stack& operator= (closable_stack const&))
...
public:
     ...
     * */
    template <typename U>
    explicit closable_stack(typename boost::allocator_rebind<node_allocator, U>::type const& alloc) :
        pool(alloc, capacity)
    {
        BOOST_STATIC_ASSERT(has_capacity);
        initialize();
     ...
     *  \pre Must specify a capacity<> argument
     * */
    explicit closable_stack(allocator const& alloc) :
        pool(alloc, capacity)
    {
        // Don't use BOOST_STATIC_ASSERT() here since it will be evaluated when compiling
        // this function and this function may be compiled even when it isn't being used.
     ...
     *  \pre Must \b not specify a capacity<> argument
     * */
    explicit closable_stack(size_type n) :
        pool(node_allocator(), n)
    {
        // Don't use BOOST_STATIC_ASSERT() here since it will be evaluated when compiling
        // this function and this function may be compiled even when it isn't being used.
     ...
     * */
    template <typename U>
    closable_stack(size_type n, typename boost::allocator_rebind<node_allocator, U>::type const& alloc) :
        pool(alloc, n)
    {
        BOOST_STATIC_ASSERT(!has_capacity);
        initialize();
...
private:
    ...
    bool link_nodes_atomic(node* new_top_node, node* end_node)
    {
        bool ok_to_push = false;
        tagged_node_handle old_tos = tos.load(detail::memory_order_relaxed);
       
        for (;;) {
            tagged_node_handle new_tos(pool.get_handle(new_top_node), old_tos.get_tag());
            end_node->next = pool.get_handle(old_tos);
            ok_to_push = new_tos.get_stat() == 0;
            if (!ok_to_push)return false;
            if (tos.compare_exchange_weak(old_tos, new_tos))
                break;
        }
        return ok_to_push;
...
private:
    ...
    bool do_push(T const& v)
    {
        ...
        if (link_nodes_atomic(newnode, newnode)) {
            return true;
        }
        pool.template destruct<true>(newnode);
        return false;
    }
...
public:
    ...
    bool consume_one(Functor& f)
    {
        ...
        for (;;) {
            ...
            tagged_node_handle new_tos(old_tos_pointer->next, old_tos.get_next_stat());
